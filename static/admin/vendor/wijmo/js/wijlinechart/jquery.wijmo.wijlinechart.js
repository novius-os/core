...
(function (wijmo) {
    var WijLineChart = (function (_super) {
        ...
        WijLineChart.prototype.destroy = function () {
        ...
        WijLineChart.prototype.getLinePath = /*****************************
        ...
        ******************************/
        ...
        function (lineIndex) {
            ...
            /// </returns>
            /// The sample shows how to get the first line.
        ...
        WijLineChart.prototype.getLineMarkers = function (lineIndex) {
            /// <summary>
            ...
            /// </returns>
            /// Code example:
            /// The sample shows how to get the markers of the first line.
        ...
        WijLineChart.prototype._showSerieEles = /** Private methods */
        function (seriesEle) {
    ...
    wijmo.WijLineChart = WijLineChart;    
    WijLineChart.prototype.options = $.extend(true, {
        ...
        /// Type: Boolean.
        /// Code example:
        /// This sample shows how to set a stacked chart.
        ...
        /// </remarks>
        /// Code example:
        /// This sample shows how to set a specified value as a data hole.
        ///  $("#linechart").wijlinechart({
        ///      hole: 100
        ///  });
        null,
        ...
        /// Code example:
        ///  $("#linechart").wijlinechart({
        ///      animation: {enabled: true, duration: 1000, easing: "easeInCubic", direction: "vertical"}
        ///  });
        /// </summary>
        {
            enabled: /// <summary>
        ...
        /// Code example:
        ///  $("#linechart").wijlinechart({
        ///      animation: {enabled: true, duration: 1000, easing: "easeOutCubic"}
        ///  });
        /// </summary>
        {
            enabled: /// <summary>
    ...
    var LineChartRender = (function () {
        ...
        LineChartRender.prototype.render = function () {
            ...
            }, seriesEles = [], culture = options.culture, widget = options.widget;
            this.widget = widget;
            ...
            if(ani.enabled || seTrans.enabled) {
                this.playAnimation(ani, seTrans, animationSet, cBounds, paths, fieldsAniPathAttr);
            }
        ...
        LineChartRender.prototype.playAnimation = function (ani, seTrans, animationSet, cBounds, paths, fieldsAniPathAttr) {
            var _this = this;
            ...
            if(ani.direction === "horizontal") {
                ...
                this.playHAnimation(duration, easing, animationSet, cBounds, paths);
            } else {
        ...
        LineChartRender.prototype.playVAnimation = function (path, fieldsAniPathAttr, idx, duration, easing) {
            ...
            if(aniPathAttr && aniPathAttr.path) {
                diffAttr = wijmo.ChartUtil.getDiffAttrs(aniPathAttr.path, path.attr());
            }
            ...
            $.each(path.markers, function (i, marker) {
                var diffMarkerAttr = wijmo.ChartUtil.getDiffAttrs(aniPathAttr.markers[i], marker.attr());
            ...
            if(path.labels) {
                $.each(path.labels, function (i, label) {
                    var diffLabelAttr = wijmo.ChartUtil.getDiffAttrs(aniPathAttr.labels[i], label.attr()), labelAttr = aniPathAttr.labels[i];
            ...
            if(path.area) {
                diffAttr = wijmo.ChartUtil.getDiffAttrs(aniPathAttr.area, path.area.attr());
                if(!$.isEmptyObject(diffAttr)) {
        ...
        LineChartRender.prototype.playHAnimation = function (duration, easing, animationSet, cBounds, paths) {
            $.each(paths, function (idx, path) {
                if(path.tracker) {
                    path.tracker.hide();
                }
            });
            var width = cBounds.endX - cBounds.startX + 20, height = cBounds.endY - cBounds.startY + 20;
            animationSet.wijAttr("clip-rect", (cBounds.startX - 10) + " " + (cBounds.startY - 10) + " 0 " + height);
            animationSet.wijAnimate({
                "clip-rect": (cBounds.startX - 10) + " " + (cBounds.startY - 10) + " " + width + " " + height
            }, duration, easing, function () {
                if(this.tracker) {
                    this.tracker.show();
                }
        ...
        LineChartRender.prototype.renderLineChart = function (options, aniPathsAttr, fieldsAniPathAttr, paths, shadowPaths, markersSet, animationSet, symbols, linesStyle, seriesEles, culture) {
            ...
            $.each(linesSeries, function (k, lineSeries) {
                ...
                if(needAnimated) {
                    if(valuesY.length > 0) {
                        firstYPoint = wijmo.ChartUtil.getFirstValidListValue(valuesY);
                        if(isYTime) {
                            firstYPoint = $.toOADate(firstYPoint);
                        }
                        lastYPoint = wijmo.ChartUtil.getLastValidListValue(valuesY);
                        if(isYTime) {
        ...
        LineChartRender.prototype.renderPath = function (canvas, bounds, lineSeries, paths, shadowPaths, linesStyle, lineHoverStyle, lineMarkerStyle, lineMarkerHoverStyle, markers, markersSet, animationSet, pathArr, aniPathsAttr, initAniPath, lineStyle, chartLabelStyle, aniMarkersAttr, aniLabelsAttr, defaultChartLabels, pathIdx, type, lastPathAttr, stacked, virtualMarkers, wijCSS) {
            ...
            //shadow
            wijmo.ChartUtil.paintShadow(path, 1, "#cccccc");
            if(pathIdx === 0) {
        ...
        LineChartRender.prototype.processYValues = function (values, display, hole, stacked, lastValues) {
            ...
            $.each(values, function (i, value) {
                ...
                idx++;
                if(wijmo.ChartUtil.isHolefunction(value, hole)) {
                    if(stacked) {
                        values[i] = 0;
                    } else {
                        val.isHole = true;
                        ...
                        if(!wijmo.ChartUtil.isHolefunction(hole, undefined) && value === hole) {
                            val.isValue = true;
                        } else {
                            val.isValue = false;
                        }
    ...
    wijmo.LineChartRender = LineChartRender;    
})(wijmo || (wijmo = {}));
