...
(function($){"use strict";$.widget("wijmo.wijdropdown",{options:{zIndex:1000,showingAnimation:{effect:"blind"},hidingAnimation:{effect:"blind"}},hoverClass:"ui-state-hover",activeClass:"ui-state-active",focusClass:"ui-state-focus",_setOption:function(key,value){$.Widget.prototype._setOption.apply(this,arguments);if(key==="disabled"){this._labelWrap.toggleClass("ui-state-disabled",value);this.element.attr("disabled",value?"disabled":"")}},_create:function(){var self=this,ele=self.element;if(ele.get(0).tagName.toLowerCase()!=="select"){return}if(ele.is(":visible")){self._activeItem=null;self._createSelect();self._bindEvents();self.needInit=false}else{self.needInit=true}},_createSelect:function(){var self=this,ele=self.element,width=ele.outerWidth(),eleWidth=width,selectWrap=ele.wrap("<div></div>").parent().addClass("ui-helper-hidden"),container=selectWrap.wrap("<div></div>").parent().attr("role","select").addClass("wijmo-wijdropdown ui-widget ui-widwijmo-wijdropdownt"+"-content ui-state-default ui-corner-all ui-helper-clearfix"),label=$("<label class=\"wijmo-dropdown-label ui-corner-all\"></label>").attr("id",ele.get(0).id+"_select").attr("name",ele.attr("name")||""),rightTrigger=$("<div></div>").addClass("wijmo-dropdown-trigger ui-state-default ui-corner-right"),labelWrap=$("<a href=\"#\"></a>"),listContainer=$("<div>").addClass("wijmo-dropdown"),list=$("<ul></ul>").addClass("wijmo-dropdown-list ui-widget-content "+"ui-widget ui-corner-all ui-helper-reset").appendTo(listContainer);$("<span></span>").addClass("ui-icon ui-icon-triangle-1-s").appendTo(rightTrigger);width=Math.max(width,container.width());if(ele.get(0).tabIndex!==""){labelWrap.attr("tabindex",ele.attr("tabindex"))}if(ele.get(0).disabled!==false){self.options.disabled=true;labelWrap.addClass("ui-state-disabled")}labelWrap.append(label);container.append(selectWrap).append(labelWrap).append(rightTrigger).append(listContainer);eleWidth+=parseInt(label.css("padding-left").replace(/px/,""),10);eleWidth+=parseInt(label.css("padding-right").replace(/px/,""),10);eleWidth-=16;container.width(eleWidth);self._buildList(list,listContainer,eleWidth);self._rightTrigger=rightTrigger;self._label=label;self._listContainer=listContainer;self._list=list;self._value=ele.val();self._selectWrap=selectWrap;self._labelWrap=labelWrap;self._container=container},_buildList:function(list,listContainer,eleWidth){var self=this,ele=self.element,height;listContainer.show();ele.children().each(function(i,n){var item=$(n),group,groupText,goupItems;if(item.is("option")){list.append(self._buildItem(item))}else{group=$("<li class=\"wijmo-dropdown-optgroup\"></li>");groupText=$("<span>"+item.attr("label")+"</span>").addClass("wijmo-optgroup-header ui-priority-primary");goupItems=$("<ul></ul>").addClass("ui-helper-reset wijmo-dropdown-items");item.children("option").each(function(){goupItems.append(self._buildItem($(this)))});group.append(groupText).append(goupItems);list.append(group)}});listContainer.height("");height=listContainer.height();height=list.outerHeight()<height?list.outerHeight():height;listContainer.css({height:height,width:eleWidth});list.setOutWidth(list.parent().parent().innerWidth()-18);if(listContainer.data("wijsuperpanel")){listContainer.wijsuperpanel("paintPanel");self.superpanel=listContainer.data("wijsuperpanel")}else{self.superpanel=listContainer.wijsuperpanel().data("wijsuperpanel")}if($.fn.bgiframe){self.superpanel.element.bgiframe()}if(!self.superpanel.vNeedScrollBar){list.setOutWidth(list.parent().parent().innerWidth());self.superpanel.refresh()}listContainer.hide()},_handelEvents:function(ele){var self=this,namespace="."+self.widgetName,element=self.element;ele.bind("click"+namespace,function(e){if(self.options.disabled){return}if(self._listContainer.is(":hidden")){self._show()}else{self._hide()}element.click();if(ele.get(0)===self._label.get(0)){e.preventDefault()}else{self._labelWrap.focus()}}).bind("mouseover"+namespace,function(){if(self.options.disabled){return}self._label.addClass(self.hoverClass);self._rightTrigger.addClass(self.hoverClass);element.trigger('mouseover')}).bind("mouseout"+namespace,function(){if(self.options.disabled){return}self._label.removeClass(self.hoverClass);self._rightTrigger.removeClass(self.hoverClass);element.trigger('mouseout')}).bind("mousedown"+namespace,function(){if(self.options.disabled){return}self._label.addClass(self.activeClass);self._rightTrigger.addClass(self.activeClass);element.trigger('mousedown')}).bind("mouseup"+namespace,function(){if(self.options.disabled){return}self._label.removeClass(self.activeClass);self._rightTrigger.removeClass(self.activeClass);element.trigger('mouseup')})},_bindEvents:function(){var self=this,namespace="."+self.widgetName,label=self._label,rightTrigger=self._rightTrigger,labelWrap=self._labelWrap,listContainer=self._listContainer,ele=self.element,offset;self._handelEvents(self._label);self._handelEvents(self._rightTrigger);$(document.body).bind("click"+namespace,function(e){if(listContainer.is(":hidden")){return}offset=listContainer.offset();if(e.target===label.get(0)||e.target===rightTrigger.get(0)||e.target===rightTrigger.children().get(0)){return}if(e.pageX<offset.left||e.pageX>offset.left+listContainer.width()||e.pageY<offset.top||e.pageY>offset.top+listContainer.height()){self._hide()}});listContainer.bind("click"+namespace,function(e){var target=$(e.target);if(target.closest("li.wijmo-dropdown-item",$(this)).length>0){self._setValue();listContainer.css("z-index","");if($.browser.msie&&/^[6,7].[0-9]+/.test($.browser.version)){listContainer.parent().css("z-index","")}listContainer.hide();self.oldVal=ele.val();ele.val(self._value);if(self.oldVal!==self._value){ele.trigger("change")}}ele.click()});labelWrap.bind("keydown"+namespace,function(e){if(self.options.disabled){return}var keyCode=$.ui.keyCode;switch(e.which){case keyCode.UP:case keyCode.LEFT:self._previous();self._setValue();self._setValueToEle();break;case keyCode.DOWN:case keyCode.RIGHT:self._next();self._setValue();self._setValueToEle();break;case keyCode.PAGE_DOWN:self._nextPage();self._setValue();self._setValueToEle();break;case keyCode.PAGE_UP:self._previousPage();self._setValue();self._setValueToEle();break;case keyCode.ENTER:case keyCode.NUMPAD_ENTER:self._setValue();self._listContainer.hide();self._setValueToEle();break}if(e.which!==keyCode.TAB){e.preventDefault()}ele.trigger('keydown')}).bind("focus"+namespace,function(){if(self.options.disabled){return}label.addClass(self.focusClass);rightTrigger.addClass(self.focusClass);ele.focus()}).bind("blur"+namespace,function(){if(self.options.disabled){return}label.removeClass(self.focusClass);rightTrigger.removeClass(self.focusClass);ele.trigger('blur')}).bind("keypress"+namespace,function(){if(self.options.disabled){return}ele.trigger('keypress')}).bind("keyup"+namespace,function(){if(self.options.disabled){return}ele.trigger('keyup')})},_init:function(){var self=this;self._initActiveItem();if(self._activeItem){self._label.text(self._activeItem.text())}},_buildItem:function($item){var val=$item.val(),text=$item.text(),self=this,$li;if(text===""){text="&nbsp;"}$li=$("<li class=\"wijmo-dropdown-item ui-corner-all\"><span>"+text+"</span></li>").mousemove(function(event){var current=$(event.target).closest(".wijmo-dropdown-item");if(current!==this.last){self._activate($(this))}this.last=$(event.target).closest(".wijmo-dropdown-item")}).attr("role","option");$li.data("value",val);return $li},_show:function(){var self=this,listContainer=self._listContainer,showingAnimation=self.options.showingAnimation;listContainer.css("z-index","100000");if($.browser.msie&&/^[6,7]\.[0-9]+/.test($.browser.version)){listContainer.parent().css("z-index","99999")}if(showingAnimation){listContainer.show(showingAnimation.effect,showingAnimation.options,showingAnimation.speed,function(){self._initActiveItem()})}else{listContainer.show()}},_hide:function(){var self=this,listContainer=self._listContainer,hidingAnimation=self.options.hidingAnimation;if(listContainer.is(":hidden")){return}if(hidingAnimation){listContainer.hide(hidingAnimation.effect,hidingAnimation.options,hidingAnimation.speed,function(){if($.isFunction(hidingAnimation.callback)){hidingAnimation.callback.apply(self,arguments)}if($.browser.msie&&/^[6,7]\.[0-9]+/.test($.browser.version)){listContainer.parent().css("z-index","")}listContainer.css("z-index","")})}else{if($.browser.msie&&$.browser.version==="6.0"){listContainer.parent().css("z-index","")}listContainer.css("z-index","");listContainer.hide()}},_setValue:function(){var self=this,listContainer=self._listContainer,top,height;if(self._activeItem){self._label.text(self._activeItem.text());self._value=self._activeItem.data("value");if(self.superpanel.vNeedScrollBar){top=self._activeItem.offset().top;height=self._activeItem.outerHeight();if(listContainer.offset().top>top){listContainer.wijsuperpanel("scrollTo",0,top-self._list.offset().top)}else if(listContainer.offset().top<top+height-listContainer.innerHeight()){listContainer.wijsuperpanel("scrollTo",0,top+height-listContainer.height()-self._list.offset().top)}}}},_setValueToEle:function(){var self=this,ele=self.element;self.oldVal=ele.val();ele.val(self._value);if(self.oldVal!==self._value){ele.trigger("change")}},_initActiveItem:function(){var self=this;if(self._value!==undefined){self._list.find("li.wijmo-dropdown-item").each(function(){if($(this).data("value")===self._value){self._activate($(this))}})}},_activate:function(item){var self=this;self._deactivate();self._activeItem=item;self._activeItem.addClass(self.hoverClass).attr("aria-selected",true)},_deactivate:function(){var self=this;if(self._activeItem){self._activeItem.removeClass(self.hoverClass).attr("aria-selected",false)}},_next:function(){this._move("next","first")},_previous:function(){this._move("prev","last")},_nextPage:function(){this._movePage("first")},_previousPage:function(){this._movePage("last")},refresh:function(){var self=this,containerWidth;if(self.needInit){if(self.element.is(":visible")){self._activeItem=null;self._createSelect();self._bindEvents();self._init();self.needInit=false}}else{if(!self._list){return}self._listContainer.show();self._selectWrap.removeClass("ui-helper-hidden");containerWidth=self.element.outerWidth();containerWidth+=parseInt(self._label.css("padding-left").replace(/px/,""),10);containerWidth+=parseInt(self._label.css("padding-right").replace(/px/,""),10);containerWidth-=16;self._container.width(containerWidth);self._selectWrap.addClass("ui-helper-hidden");self._list.empty();self._buildList(self._list,self._listContainer,containerWidth);self._value=self.element.val();self._initActiveItem();if(self._activeItem){self._label.text(self._activeItem.text())}}},_move:function(direction,edge){var self=this,$nextLi,next;if(!self._activeItem){self._activate(self._list.find(".wijmo-dropdown-item:"+edge));return}$nextLi=self._activeItem[direction]().eq(0);if($nextLi.length){next=self._getNextItem($nextLi,direction,edge)}else if(self._activeItem.closest(".wijmo-dropdown-optgroup").length){next=self._getNextItem(self._activeItem.closest(".wijmo-dropdown-optgroup")[direction](),direction,edge)}if(next&&next.length){self._activate(next)}else{self._activate(self._list.find(".wijmo-dropdown-item:"+edge))}},_movePage:function(direction){var self=this,base,height,result,antiDirection=direction==="first"?"last":"first";if(self.superpanel.vNeedScrollBar){base=self._activeItem.offset().top;height=self.options.height;result=self._list.find(".wijmo-dropdown-item").filter(function(){var close=$(this).offset().top-base+(direction==="first"?-height:height)+$(this).height(),lineheight=$(this).height();return close<lineheight&&close>-lineheight});if(!result.length){result=self._list.find(".wijmo-dropdown-item:"+antiDirection)}self._activate(result)}else{self._activate(self._list.find(".wijmo-dropdown-item:"+(!self._activeItem?direction:antiDirection)))}},_getNextItem:function(next,direction,edge){if(next.length){if(next.is(".wijmo-dropdown-optgroup")){if(!!next.find(">ul>li.wijmo-dropdown-item").length){return next.find(">ul>li.wijmo-dropdown-item:"+edge).eq(0)}else{this._getNextItem(next[direction]().eq(0))}}else{return next}}},destroy:function(){this.element.closest(".wijmo-wijdropdown").find(">div.wijmo-dropdown-trigger,>div.wijmo-dropdown,"+">a").remove();this.element.unwrap().unwrap().removeData("maxZIndex");$.Widget.prototype.destroy.apply(this)}})}(jQuery));
...
(function($){"use strict";var tabId=0,listId=0;function getNextTabId(){return++tabId}function getNextListId(){return++listId}$.widget("wijmo.wijtabs",{options:{alignment:'top',sortable:false,scrollable:false,ajaxOptions:null,cache:false,cookie:null,collapsible:false,hideOption:null,showOption:null,disabledIndexes:[],event:'click',idPrefix:'ui-tabs-',panelTemplate:'',spinner:'',tabTemplate:'',add:null,remove:null,select:null,beforeShow:null,show:null,load:null,disable:null,enable:null},_defaults:{panelTemplate:'<div></div>',spinner:'<em>Loading&#8230;</em>',tabTemplate:'<li><a href="#{href}"><span>#{label}</span></a></li>'},_create:function(){this._tabify(true)},_setOption:function(key,value){$.Widget.prototype._setOption.apply(this,arguments);switch(key){case'selected':if(this.options.collapsible&&value==this.options.selected){return}this.select(value);break;case'alignment':this.destroy();this._tabify(true);break;default:this._tabify();break}},_initScroller:function(){var horz=$.inArray(this._getAlignment(),['top','bottom'])!=-1;if(!horz){return}var width=0;this.lis.each(function(){width+=$(this).outerWidth(true)});if(!!this.options.scrollable&&this.element.innerWidth()<width){if(this.scrollWrap===undefined){this.list.wrap("<div class='scrollWrap'></div>");this.scrollWrap=this.list.parent();$.effects.save(this.list,['width','height','overflow'])}this.list.width(width+2);this.scrollWrap.height(this.list.outerHeight(true));this.scrollWrap.wijsuperpanel({allowResize:false,hScroller:{scrollMode:'edge'},vScroller:{scrollBarVisibility:'hidden'}})}else{this._removeScroller()}},_removeScroller:function(){if(this.scrollWrap){this.scrollWrap.wijsuperpanel('destroy').replaceWith(this.scrollWrap.contents());this.scrollWrap=undefined;$.effects.restore(this.list,['width','height','overflow'])}},_tabId:function(a){return a.title&&a.title.replace(/\s/g,'_').replace(/[^A-Za-z0-9\-_:\.]/g,'')||this.options.idPrefix+getNextTabId()},_sanitizeSelector:function(hash){return hash.replace(/:/g,'\\:')},_cookie:function(){var cookie=this.cookie||(this.cookie=this.options.cookie.name||'ui-tabs-'+getNextListId());return $.cookie.apply(null,[cookie].concat($.makeArray(arguments)))},_ui:function(tab,panel){return{tab:tab,panel:panel,index:this.anchors.index(tab)}},_cleanup:function(){this.lis.filter('.ui-state-processing').removeClass('ui-state-processing').find('span:data(label.tabs)').each(function(){var el=$(this);el.html(el.data('label.tabs')).removeData('label.tabs')})},_getAlignment:function(tabs){tabs=tabs===undefined?true:tabs;var align=this.options.alignment||'top';if(tabs){return align}switch(align){case'top':align='bottom';break;case'bottom':align='top';break;case'left':align='right';break;case'right':align='left';break}return align},_saveLayout:function(){var props=['width','height','overflow'];$.effects.save(this.element,props);$.effects.save(this.list,props);$.effects.save(this.element.find('.wijmo-wijtabs-content'),props);this.list.width(this.list.width());var $hide=this.panels.filter(':not(.ui-tabs-hide)');this.element.data('panel.width',$hide.width());this.element.data('panel.outerWidth',$hide.outerWidth(true))},_restoreLayout:function(){var props=['width','height','overflow'];$.effects.restore(this.element,props);$.effects.restore(this.list,props);$.effects.restore(this.element.find('.wijmo-wijtabs-content'),props)},_hideContent:function(){var content=this.element.find('.wijmo-wijtabs-content');if(content.length){this._saveLayout();content.addClass('ui-tabs-hide').attr('aria-hidden',true);this.element.width(this.list.outerWidth(true))}},_showContent:function(){var content=this.element.find('.wijmo-wijtabs-content');if(content.length){this._restoreLayout();content.removeClass('ui-tabs-hide').attr('aria-hidden',false)}},_blindPanel:function(panel,mode){var o=this.options;var content=panel.parent('.wijmo-wijtabs-content');if(!content.length){return}this.list.width(this.list.width());var props=['position','top','left','width'];$.effects.save(panel,props);panel.show();if(mode=='show'){panel.removeClass('ui-tabs-hide').attr('aria-hidden',false);panel.width(this.element.data('panel.width'))}else{panel.width(panel.width())}var blindOption=mode=='show'?o.showOption:o.hideOption;var wrapper=$.effects.createWrapper(panel).css({overflow:'hidden'});if(mode=='show'){wrapper.css($.extend({width:0},blindOption.fade?{opacity:0}:{}))}var a=$.extend({width:mode=='show'?this.element.data('panel.outerWidth'):0},blindOption.fade?{opacity:mode=='show'?1:0}:{});var self=this;var listWidth=this.list.outerWidth(true);wrapper.animate(a,{duration:blindOption.duration,step:function(){var ww=wrapper.outerWidth(true);self.element.width(listWidth+ww);content.width(Math.max(0,self.element.innerWidth()-listWidth-6))},complete:function(){if(mode=='hide'){self.lis.removeClass('ui-tabs-selected ui-state-active').attr('aria-selected',false);panel.addClass('ui-tabs-hide').attr('aria-hidden',true)}else{panel.css('width','')}$.effects.removeWrapper(panel);if(mode=='show'){self._restoreLayout()}self._resetStyle(panel);panel.dequeue();self.element.dequeue("tabs")}})},_resetStyle:function($el){$el.css({display:''});if(!$.support.opacity){$el[0].style.removeAttribute('filter')}},_normalizeBlindOption:function(o){if(o.blind===undefined){o.blind=false}if(o.fade===undefined){o.fade=false}if(o.duration===undefined){o.duration=200}if(typeof o.duration=='string'){try{o.duration=parseInt(o.duration,10)}catch(e){o.duration=200}}},_tabify:function(init){this.list=this.element.children('ol,ul').eq(0);this.lis=$('li:has(a)',this.list);this.anchors=this.lis.map(function(){return $('a',this)[0]});this.panels=$([]);var self=this,o=this.options;var fragmentId=/^#.+/;this.anchors.each(function(i,a){var href=$(a).attr('href')||"";var hrefBase=href.split('#')[0],baseEl;if(hrefBase&&(hrefBase===location.toString().split('#')[0]||(baseEl=$('base')[0])&&hrefBase===baseEl.href)){href=a.hash;a.href=href}if(fragmentId.test(href)){self.panels=self.panels.add(self._sanitizeSelector(href))}else if(href!='#'){$.data(a,'href.tabs',href);$.data(a,'load.tabs',href.replace(/#.*$/,''));var id=self._tabId(a);a.href='#'+id;var $panel=$('#'+id);if(!$panel.length){$panel=$(o.panelTemplate||self._defaults.panelTemplate).attr('id',id).addClass('ui-tabs-panel ui-widget-content ui-corner-bottom').insertAfter(self.panels[i-1]||self.list);$panel.data('destroy.tabs',true)}self.panels=self.panels.add($panel)}else{o.disabledIndexes.push(i)}});var tabsAlign=this._getAlignment(),panelCorner=this._getAlignment(false);if(init){this.list.attr("role","tablist");this.lis.attr("role","tab");this.panels.attr("role","tabpanel");this.element.addClass('ui-tabs wijmo-wijtabs'+' ui-tabs-'+tabsAlign+' ui-widget ui-widget-content ui-corner-all ui-helper-clearfix');this.list.addClass('ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all');this.lis.addClass('ui-state-default'+' ui-corner-'+tabsAlign);this.panels.addClass('ui-tabs-panel ui-widget-content ui-corner-'+panelCorner);var content;switch(tabsAlign){case'bottom':this.list.appendTo(this.element);break;case'left':content=$('<div/>').addClass('wijmo-wijtabs-content').appendTo(this.element);this.panels.appendTo(content);break;case'right':content=$('<div/>').addClass('wijmo-wijtabs-content').insertBefore(this.list);this.panels.appendTo(content);break;case'top':this.list.prependTo(this.element);break}if(o.sortable){this.list.sortable({axis:(tabsAlign=='top'||tabsAlign=='bottom')?"x":"y"})}if(o.selected===undefined){if(location.hash){this.anchors.each(function(i,a){if(a.hash==location.hash){o.selected=i;return false}})}if(typeof o.selected!='number'&&o.cookie){o.selected=parseInt(self._cookie(),10)}if(typeof o.selected!='number'&&this.lis.filter('.ui-tabs-selected').length){o.selected=this.lis.index(this.lis.filter('.ui-tabs-selected'))}o.selected=o.selected||(this.lis.length?0:-1)}else if(o.selected===null){o.selected=-1}o.selected=((o.selected>=0&&this.anchors[o.selected])||o.selected<0)?o.selected:0;o.disabledIndexes=$.unique(o.disabledIndexes.concat($.map(this.lis.filter('.ui-state-disabled'),function(n,i){return self.lis.index(n)}))).sort();if($.inArray(o.selected,o.disabledIndexes)!=-1){o.disabledIndexes.splice($.inArray(o.selected,o.disabledIndexes),1)}this.panels.addClass('ui-tabs-hide').attr('aria-hidden',true);this.lis.removeClass('ui-tabs-selected ui-state-active').attr('aria-selected',false);if(o.selected>=0&&this.anchors.length){this.panels.eq(o.selected).removeClass('ui-tabs-hide').attr('aria-hidden',false);this.lis.eq(o.selected).addClass('ui-tabs-selected ui-state-active').attr('aria-selected',true);self.element.queue("tabs",function(){self._trigger('show',null,self._ui(self.anchors[o.selected],self.panels[o.selected]))});this.load(o.selected)}$(window).bind('unload',function(){if(self.lis){self.lis.add(self.anchors).unbind('.tabs')}self.lis=self.anchors=self.panels=null})}else{o.selected=this.lis.index(this.lis.filter('.ui-tabs-selected'))}this.element[o.collapsible?'addClass':'removeClass']('ui-tabs-collapsible');if(o.cookie){this._cookie(o.selected,o.cookie)}for(var i=0,li;(li=this.lis[i]);i++){$(li)[$.inArray(i,o.disabledIndexes)!=-1&&!$(li).hasClass('ui-tabs-selected')?'addClass':'removeClass']('ui-state-disabled');if($(li).hasClass('ui-state-disabled')){$(li).attr('aria-disabled',true)}}if(o.cache===false){this.anchors.removeData('cache.tabs')}this.lis.add(this.anchors).unbind('.tabs');if(!o.disabled&&o.event!='mouseover'){var addState=function(state,el){if(el.is(':not(.ui-state-disabled)')){el.addClass('ui-state-'+state)}};var removeState=function(state,el){el.removeClass('ui-state-'+state)};this.lis.bind('mouseover.tabs',function(){addState('hover',$(this))});this.lis.bind('mouseout.tabs',function(){removeState('hover',$(this))});this.anchors.bind('focus.tabs',function(){addState('focus',$(this).closest('li'))});this.anchors.bind('blur.tabs',function(){removeState('focus',$(this).closest('li'))})}if(o.showOption===undefined||o.showOption===null){o.showOption={}}this._normalizeBlindOption(o.showOption);if(o.hideOption===undefined||o.hideOption===null){o.hideOption={}}this._normalizeBlindOption(o.hideOption);var showTab=((o.showOption.blind||o.showOption.fade)&&o.showOption.duration>0)?function(clicked,$show){$(clicked).closest('li').addClass('ui-tabs-selected ui-state-active').attr('aria-selected',true);self._showContent();$show.removeClass('ui-tabs-hide').attr('aria-hidden',false);if(tabsAlign=='top'||tabsAlign=='bottom'){var props={duration:o.showOption.duration};if(o.showOption.blind){props.height='toggle'}if(o.showOption.fade){props.opacity='toggle'}$show.hide().removeClass('ui-tabs-hide').attr('aria-hidden',false).animate(props,o.showOption.duration||'normal',function(){self._resetStyle($show);self._trigger('show',null,self._ui(clicked,$show[0]))})}else{self._showContent();self._blindPanel($show,'show')}}:function(clicked,$show){$(clicked).closest('li').addClass('ui-tabs-selected ui-state-active').attr('aria-selected',true);self._showContent();$show.removeClass('ui-tabs-hide').attr('aria-hidden',false);self._trigger('show',null,self._ui(clicked,$show[0]))};var hideTab=((o.hideOption.blind||o.hideOption.fade)&&o.hideOption.duration>0)?function(clicked,$hide){if(tabsAlign=='top'||tabsAlign=='bottom'){var props={duration:o.hideOption.duration};if(o.hideOption.blind){props.height='toggle'}if(o.hideOption.fade){props.opacity='toggle'}$hide.animate(props,o.hideOption.duration||'normal',function(){self.lis.removeClass('ui-tabs-selected ui-state-active').attr('aria-selected',false);$hide.addClass('ui-tabs-hide').attr('aria-hidden',true);self._resetStyle($hide);self.element.dequeue("tabs")})}else{self._saveLayout();self._blindPanel($hide,'hide')}}:function(clicked,$hide,$show){self.lis.removeClass('ui-tabs-selected ui-state-active').attr('aria-selected',false);self._hideContent();$hide.addClass('ui-tabs-hide').attr('aria-hidden',true);self.element.dequeue("tabs")};if(!o.disabled){this.anchors.bind(o.event+'.tabs',function(){var el=this,$li=$(this).closest('li'),$hide=self.panels.filter(':not(.ui-tabs-hide)'),$show=$(self._sanitizeSelector(this.hash));if(($li.hasClass('ui-tabs-selected')&&!o.collapsible)||$li.hasClass('ui-state-disabled')||$li.hasClass('ui-state-processing')||self._trigger('select',null,self._ui(this,$show[0]))===false){this.blur();return false}o.selected=self.anchors.index(this);self.abort();if(o.collapsible){if($li.hasClass('ui-tabs-selected')){o.selected=-1;if(o.cookie){self._cookie(o.selected,o.cookie)}self.element.queue("tabs",function(){hideTab(el,$hide)}).dequeue("tabs");this.blur();return false}else if(!$hide.length){if(o.cookie){self._cookie(o.selected,o.cookie)}self.element.queue("tabs",function(){showTab(el,$show)});self.load(self.anchors.index(this));this.blur();return false}}if(o.cookie){self._cookie(o.selected,o.cookie)}if($show.length){if($hide.length){self.element.queue("tabs",function(){hideTab(el,$hide)})}self.element.queue("tabs",function(){showTab(el,$show)});self.load(self.anchors.index(this))}else{throw'jQuery UI Tabs: Mismatching fragment identifier.'}if($.browser.msie){this.blur()}})}this._initScroller();this.anchors.bind('click.tabs',function(){return false})},destroy:function(){var o=this.options;this.abort();this._removeScroller();this.element.unbind('.tabs').removeClass(['wijmo-wijtabs','ui-tabs-top','ui-tabs-bottom','ui-tabs-left','ui-tabs-right','ui-tabs','ui-widget','ui-widget-content','ui-corner-all','ui-tabs-collapsible','ui-helper-clearfix'].join(' ')).removeData('tabs').removeAttr('role');this.list.removeClass('ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all').removeAttr('role');this.anchors.each(function(){var href=$.data(this,'href.tabs');if(href){this.href=href}var $this=$(this).unbind('.tabs');$.each(['href','load','cache'],function(i,prefix){$this.removeData(prefix+'.tabs')})});this.lis.unbind('.tabs').add(this.panels).each(function(){if($.data(this,'destroy.tabs')){$(this).remove()}else{$(this).removeClass(['ui-state-default','ui-corner-top','ui-corner-bottom','ui-corner-left','ui-corner-right','ui-tabs-selected','ui-state-active','ui-state-hover','ui-state-focus','ui-state-disabled','ui-tabs-panel','ui-widget-content','ui-tabs-hide'].join(' ')).css({position:'',left:'',top:''}).removeAttr('role').removeAttr('aria-hidden').removeAttr('aria-selected').removeAttr('aria-disabled')}});var content=this.element.find('.wijmo-wijtabs-content');if(content.length){content.replaceWith(content.contents())}if(o.cookie){this._cookie(null,o.cookie)}return this},add:function(url,label,index){if(index===undefined){index=this.anchors.length}var self=this,o=this.options,$li=$((o.tabTemplate||self._defaults.tabTemplate).replace(/#\{href\}/g,url).replace(/#\{label\}/g,label)),id=!url.indexOf('#')?url.replace('#',''):this._tabId($('a',$li)[0]);var tabsAlign=this._getAlignment(),panelCorner=this._getAlignment(false);$li.addClass('ui-state-default'+' ui-corner-'+tabsAlign).data('destroy.tabs',true).attr('role','tab').attr('aria-selected',false);var $panel=$('#'+id);if(!$panel.length){$panel=$(o.panelTemplate||self._defaults.panelTemplate).attr('id',id).data('destroy.tabs',true).attr('role','tabpanel')}$panel.addClass('ui-tabs-panel ui-widget-content ui-corner-'+panelCorner+' ui-tabs-hide').attr('aria-hidden',true);if(index>=this.lis.length){$li.appendTo(this.list);if(this.panels.length>0){$panel.insertAfter(this.panels[this.panels.length-1])}else{var $content=this.element.find('.wijmo-wijtabs-content');if($content.length===0)$content=this.element;$panel.appendTo($content)}}else{$li.insertBefore(this.lis[index]);$panel.insertBefore(this.panels[index])}o.disabledIndexes=$.map(o.disabledIndexes,function(n,i){return n>=index?++n:n});this._removeScroller();this._tabify();if(this.anchors.length==1){o.selected=0;$li.addClass('ui-tabs-selected ui-state-active').attr('aria-selected',true);$panel.removeClass('ui-tabs-hide').attr('aria-hidden',false);this.element.queue("tabs",function(){self._trigger('show',null,self._ui(self.anchors[0],self.panels[0]))});this.load(0)}this._trigger('add',null,this._ui(this.anchors[index],this.panels[index]));return this},remove:function(index){var o=this.options,$li=this.lis.eq(index).remove(),$panel=this.panels.eq(index).remove();if($li.hasClass('ui-tabs-selected')&&this.anchors.length>1){this.select(index+(index+1<this.anchors.length?1:-1))}o.disabledIndexes=$.map($.grep(o.disabledIndexes,function(n,i){return n!=index}),function(n,i){return n>=index?--n:n});this._removeScroller();this._tabify();this._trigger('remove',null,this._ui($li.find('a')[0],$panel[0]));return this},enableTab:function(index){var o=this.options;if($.inArray(index,o.disabledIndexes)==-1){return}this.lis.eq(index).removeClass('ui-state-disabled').removeAttr('aria-disabled');o.disabledIndexes=$.grep(o.disabledIndexes,function(n,i){return n!=index});this._trigger('enable',null,this._ui(this.anchors[index],this.panels[index]));return this},disableTab:function(index){var self=this,o=this.options;if(index!=o.selected){this.lis.eq(index).addClass('ui-state-disabled').attr('aria-disabled',true);o.disabledIndexes.push(index);o.disabledIndexes.sort();this._trigger('disable',null,this._ui(this.anchors[index],this.panels[index]))}return this},select:function(index){if(typeof index=='string'){index=this.anchors.index(this.anchors.filter('[href$='+index+']'))}else if(index===null){index=-1}if(index==-1&&this.options.collapsible){index=this.options.selected}this.anchors.eq(index).trigger(this.options.event+'.tabs');return this},load:function(index){var self=this,o=this.options,a=this.anchors.eq(index)[0],url=$.data(a,'load.tabs');this.abort();if(!url||this.element.queue("tabs").length!==0&&$.data(a,'cache.tabs')){this.element.dequeue("tabs");return}this.lis.eq(index).addClass('ui-state-processing');if(o.spinner||self._defaults.spinner){var span=$('span',a);span.data('label.tabs',span.html()).html(o.spinner||self._defaults.spinner)}this.xhr=$.ajax($.extend({},o.ajaxOptions,{url:url,success:function(r,s){$(self._sanitizeSelector(a.hash)).html(r);self._cleanup();if(o.cache){$.data(a,'cache.tabs',true)}self._trigger('load',null,self._ui(self.anchors[index],self.panels[index]));try{o.ajaxOptions.success(r,s)}catch(e1){}},error:function(xhr,s,e){self._cleanup();self._trigger('load',null,self._ui(self.anchors[index],self.panels[index]));try{o.ajaxOptions.error(xhr,s,index,a)}catch(e2){}}}));self.element.dequeue("tabs");return this},abort:function(){this.element.queue([]);this.panels.stop(false,true);this.element.queue("tabs",this.element.queue("tabs").splice(-2,2));if(this.xhr){this.xhr.abort();delete this.xhr}this._cleanup();return this},url:function(index,url){this.anchors.eq(index).removeData('cache.tabs').data('load.tabs',url);return this},length:function(){return this.anchors.length}})})(jQuery);
