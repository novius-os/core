 ...
 */
define("jquery-nos",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete"],function(a){var c=void (0);a.nos={dispatchEvent:function(d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.dispatchEvent(d)}if(a.nos.$noviusos){a.nos.$noviusos.ostabs("dispatchEvent",d)}},dataStore:{},data:function(e,d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.data(e,d)}if(e){if(d){this.dataStore[e]=d}return this.dataStore[e]}},notify:function(d,e){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.notify(d,e)}if(!a.isPlainObject(d)){d={title:d}}if(e!==c){a.extend(d,a.isPlainObject(e)?e:{type:e})}if(a.isPlainObject(d)){require(["link!static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.default.css","static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.min"],function(){var f={pnotify_history:false,pnotify_addclass:"nos-notification"};a.each(d,function(g,h){if(g.substr(0,8)!=="pnotify_"){g="pnotify_"+g}f[g]=h});return a.pnotify(f)})}return false},ajax:{request:function(d){d=a.extend({dataType:"json",type:"POST",data:{}},d);if(d.dataType=="json"){if(a.isFunction(d.success)){var f=d.success;d.success=function(g){g.user_success=f;a.nos.ajax.success(g)}}else{d.success=a.nos.ajax.success}if(a.isFunction(d.error)){var e=d.error;d.error=function(g){a.nos.ajax.error(g);e.apply(this,arguments)}}else{d.error=a.nos.ajax.error}}a.ajax(d)},success:function(d){if(d.error){if(a.isArray(d.error)){a.each(d.error,function(){a.nos.notify(this,"error")})}else{a.nos.notify(d.error,"error")}}if(d.notify){if(a.isArray(d.notify)){a.each(d.notify,function(){a.nos.notify(this)})}else{a.nos.notify(d.notify)}}if(d.dispatchEvent){if(a.isArray(d.dispatchEvent)){a.each(d.dispatchEvent,function(e,f){a.nos.dispatchEvent(f)})}else{a.nos.dispatchEvent(d.dispatchEvent)}}if(a.isFunction(d.user_success)){d.user_success.apply(this,arguments)}if(d.redirect){document.location=d.redirect}if(d.closeTab){a.nos.tabs.close()}if(d.replaceTab){a.nos.tabs.replace(d.replaceTab)}},error:function(d,f){if(d.status!=0){a.nos.notify("Connection error!","error")}else{if(f=="parsererror"){a.nos.notify("Request seemed a success, but we could not read the answer.")}else{if(f=="timeout"){a.nos.notify("Time out (server is busy?). Please try again.")}}}}},media:function(e,h){var d={all:"/admin/nos/media/list",image:"/admin/nos/media/list?view=image_pick"};var g=null;var f=a.extend({title:e.attr("title")||"File",allowDelete:true,choose:function(i){if(g==null){g=a(e).noviusos().dialogOpen({contentUrl:d[h.mode],ajax:true,title:"Choose a media file"});g.bind("select.media",function(k,j){e.inputFileThumb({file:j.thumbnail});e.val(j.id);g.wijdialog("close")})}else{g.wijdialog("open")}}},h.inputFileThumb);require(["static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/js/jquery.input-file-thumb","link!static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/css/jquery.input-file-thumb.css"],function(){a(function(){e.inputFileThumb(f)})})},form:{ajax:function(d){if(!d){return}var e=a(d);if(!e.is("form")){e=e.find("form")}a.nos.ui.validate(e,{})}},ui:{form:function(d){d=d||"body";a(function(){var e=a(d);e.find(":input[type='text'],:input[type='password'],:input[type='email'],textarea").wijtextbox();e.find(":input[type='submit'],button").each(function(){var f={};var g=a(this).data("icon");if(g){f.icons={primary:"ui-icon-"+g}}a(this).button(f)});e.find("select").filter(":not(.notransform)").wijdropdown();e.find(":input[type=checkbox]").wijcheckbox();e.find(".expander").each(function(){var f=a(this);f.wijexpander(a.extend({expanded:true},f.data("wijexpander-options")))});e.find(".accordion").wijaccordion({header:"h3",selectedIndexChanged:function(g,f){a.nos.ui.initOnShow.show(a(g.target).find(".ui-accordion-content").eq(f.newIndex))}})})},initOnShow:{init:function(d,e){if(!a.isFunction(e)){return}if(d.is(":visible")){e()}else{d.addClass("nos-init-on-show").data("nos-init-on-show",e)}},show:function(d){d.find(".nos-init-on-show:visible").each(function(){var e=a(this),f=e.data("nos-init-on-show");e.removeClass("nos-init-on-show");f()})}},validate:function(e,f){var d=function(g){require(["jquery-form"],function(){a(g).ajaxSubmit({dataType:"json",success:function(h){a.nos.ajax.success(h);a(g).triggerHandler("ajax_success",[h])},error:function(){a.nos.notify("An error occured","error")}})})};a(function(){if(!f||a.isEmptyObject(f)){a(e).submit(function(g){d(e);g.preventDefault()})}else{require(["jquery-validate"],function(){a(e).validate(a.extend({},f,{errorClass:"ui-state-error",success:true,ignore:"input[type=hidden]",submitHandler:d}))})}})}},saveUserConfiguration:function(d,e){this.ajax.request({url:"/admin/nos/noviusos/save_user_configuration",data:{key:d,configuration:e}})}};var b=window.$nos=a.sub();b.fn.extend({dialogOpen:function(e){if(e.destroyOnClose){var f=e.close;e.close=function(){if(a.isFunction(f)){f.apply(this,arguments)}a(this).wijdialog("destroy").remove()}}e=a.extend(true,{},{width:window.innerWidth-200,height:window.innerHeight-100,modal:true,captionButtons:{pin:{visible:false},refresh:{visible:e.contentUrl!=null&&!e.ajax},toggle:{visible:false},minimize:{visible:false},maximize:{visible:false}}},e);var d=a.nos.$noviusos.ostabs?a.nos.$noviusos.ostabs("current").panel:a("body");var h=a(document.createElement("div")).appendTo(d);a.nos.data("dialog",h);if(typeof e.content!="undefined"){h.append(e.content)}var g=true;if(e.ajax){var i=e.contentUrl;delete e.contentUrl;e.autoOpen=false;h.wijdialog(e);a.ajax({url:i,type:"GET",dataType:"html",complete:function(l,j,m){m=l.responseText;if(l.isResolved()){l.done(function(o){m=o});try{var k=a.parseJSON(m);g=false}catch(n){}if(g){h.wijdialog("open")}else{h.empty();h.wijdialog("destroy");h.remove();a.nos.ajax.success(k)}h.html(m)}}})}else{h.wijdialog(e)}if(g){if(a.isFunction(e.onLoad)){e.onLoad()}h.bind("wijdialogclose",function(j,k){h.closest(".ui-dialog").hide().appendTo(d)})}return h},dialogClose:function(){this.closest(":wijmo-wijdialog").wijdialog("close").wijdialog("destroy").remove()},tabOpen:function(f,e){var d=this.closest(".ui-dialog-content").size();if(d){e=e||{};this.dialogOpen(a.extend({contentUrl:f.url,ajax:!f.iframe,title:f.label},e))}else{a.nos.tabs.add(f)}return this},tabClose:function(){var d=this.closest(":wijmo-wijdialog");if(d.size()){this.dialogClose()}else{a.nos.tabs.close()}return this}});a.fn.noviusos=function(){return b(this)};return b});
