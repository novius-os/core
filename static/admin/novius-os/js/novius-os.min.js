 ...
 */
define("jquery-nos",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete"],function(a){var c=void (0);a.nos={dispatchEvent:function(d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.dispatchEvent(d)}if(a.nos.$noviusos){a.nos.$noviusos.ostabs("dispatchEvent",d)}},ajax:{request:function(d){d=a.extend({dataType:"json",type:"POST",data:{}},d);if(d.dataType=="json"){if(a.isFunction(d.success)){var f=d.success;d.success=function(g){g.user_success=f;a.nos.ajax.success(g)}}else{d.success=a.nos.ajax.success}if(a.isFunction(d.error)){var e=d.error;d.error=function(g){a.nos.ajax.error(g);e.apply(this,arguments)}}else{d.error=a.nos.ajax.error}}a.ajax(d)},success:function(d){if(d.error){if(a.isArray(d.error)){a.each(d.error,function(){b.notify(this,"error")})}else{b.notify(d.error,"error")}}if(d.notify){if(a.isArray(d.notify)){a.each(d.notify,function(){b.notify(this)})}else{b.notify(d.notify)}}if(d.dispatchEvent){if(a.isArray(d.dispatchEvent)){a.each(d.dispatchEvent,function(e,f){a.nos.dispatchEvent(f)})}else{a.nos.dispatchEvent(d.dispatchEvent)}}if(a.isFunction(d.user_success)){d.user_success.apply(this,arguments)}if(d.redirect){document.location=d.redirect}if(d.closeTab){a.nos.tabs.close()}if(d.replaceTab){a.nos.tabs.replace(d.replaceTab)}},error:function(d,f){if(d.status!=0){b.notify("Connection error!","error")}else{if(f=="parsererror"){b.notify("Request seemed a success, but we could not read the answer.")}else{if(f=="timeout"){b.notify("Time out (server is busy?). Please try again.")}}}}},saveUserConfiguration:function(d,e){this.ajax.request({url:"/admin/nos/noviusos/save_user_configuration",data:{key:d,configuration:e}})}};var b=window.$nos=a.sub();b.extend({notify:function(d,e){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.notify(d,e)}if(!a.isPlainObject(d)){d={title:d}}if(e!==c){a.extend(d,a.isPlainObject(e)?e:{type:e})}if(a.isPlainObject(d)){require(["link!static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.default.css","static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.min"],function(){var f={pnotify_history:false,pnotify_addclass:"nos-notification"};a.each(d,function(g,h){if(g.substr(0,8)!=="pnotify_"){g="pnotify_"+g}f[g]=h});return a.pnotify(f)})}return false}});b.fn.extend({media:function(g){g=g||{};var d={all:"/admin/nos/media/list",image:"/admin/nos/media/list?view=image_pick"},h=this;var f=null;var e=a.extend({title:h.attr("title")||"File",allowDelete:true,choose:function(i){if(f===null){f=h.dialog({destroyOnClose:false,contentUrl:d[g.mode],ajax:true,title:"Choose a media file"});f.bind("select.media",function(k,j){h.inputFileThumb({file:j.thumbnail});h.val(j.id);f.wijdialog("close")})}else{f.wijdialog("open")}}},g.inputFileThumb||{});require(["static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/js/jquery.input-file-thumb","link!static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/css/jquery.input-file-thumb.css"],function(){a(function(){h.inputFileThumb(e)})})},form:function(){var f=Array.prototype.slice.call(arguments),h="ui";if(f.length>0&&a.inArray(f[0],["ui","validate","ajax"])!==-1){h=f.shift()}switch(h){case"ui":var e=this;e.find(":input[type='text'],:input[type='password'],:input[type='email'],textarea").wijtextbox();e.find(":input[type='submit'],button").each(function(){var i={};var j=a(this).data("icon");if(j){i.icons={primary:"ui-icon-"+j}}a(this).button(i)});e.find("select").filter(":not(.notransform)").nos().initOnShow("init",function(){a(this).wijdropdown()});e.find(":input[type=checkbox]").nos().initOnShow("init",function(){a(this).wijcheckbox()});e.find(".expander").each(function(){var i=a(this);i.wijexpander(a.extend({expanded:true},i.data("wijexpander-options")))});e.find(".accordion").wijaccordion({header:"h3",selectedIndexChanged:function(j,i){a(j.target).find(".ui-accordion-content").eq(i.newIndex).nos().initOnShow()}});break;case"validate":var d=this,g=f[0]||{};if(!d.is("form")){d=d.find("form")}require(["jquery-validate"],function(){d.validate(a.extend({},g,{errorClass:"ui-state-error",success:true,ignore:"input[type=hidden]"}))});break;case"ajax":var d=this;if(!d.is("form")){d=d.find("form")}require(["jquery-form"],function(){d.ajaxForm({dataType:"json",success:function(i){a.nos.ajax.success(i);d.triggerHandler("ajax_success",[i])},error:function(){b.notify("An error occured","error")}})});break}return this},initOnShow:function(){var d=Array.prototype.slice.call(arguments),f="show";if(d.length>0&&a.inArray(d[0],["init","show"])!==-1){f=d.shift()}switch(f){case"init":var e=d[0];if(!a.isFunction(e)){return}this.each(function(){var g=a(this);if(g.is(":visible")){e.call(g)}else{g.addClass("nos-init-on-show").data("nos-init-on-show",e)}});break;case"show":this.find(".nos-init-on-show:visible").each(function(){var g=a(this),h=g.data("nos-init-on-show");g.removeClass("nos-init-on-show");h.call(this)});break}return this},dialog:function(){var j=Array.prototype.slice.call(arguments),d="open";if(j.length>0&&a.inArray(j[0],["open","close"])!==-1){d=j.shift()}switch(d){case"open":var l=j[0]||{},m=a.extend(true,{},{destroyOnClose:true,width:window.innerWidth-200,height:window.innerHeight-100,modal:true,captionButtons:{pin:{visible:false},refresh:{visible:l.contentUrl!=null&&!l.ajax},toggle:{visible:false},minimize:{visible:false},maximize:{visible:false}}},l),e=m.close,h=m.open,k=this.closest(".nos-dispatcher, body"),f=a("<div></div>").appendTo(k);m.close=function(){if(a.isFunction(e)){e.apply(f,arguments)}if(m.destroyOnClose){f.wijdialog("destroy").remove()}else{f.closest(".ui-dialog").hide().appendTo(k)}};if(!m.destroyOnClose){m.open=function(){if(a.isFunction(h)){h.apply(f,arguments)}f.closest(".ui-dialog").appendTo("body")}}if(m.content!==c){f.append(m.content)}var i=true;if(m.ajax){var g=m.contentUrl;delete m.contentUrl;m.autoOpen=false;f.wijdialog(m);a.ajax({url:g,type:"GET",dataType:"html",complete:function(p,n,q){q=p.responseText;if(p.isResolved()){p.done(function(s){q=s});try{var o=a.parseJSON(q);i=false}catch(r){}if(i){f.wijdialog("open").html(q)}else{f.empty().wijdialog("destroy").remove();a.nos.ajax.success(o)}}}})}else{f.wijdialog(m)}if(i&&a.isFunction(m.onLoad)){m.onLoad()}return f;break;case"close":this.closest(":wijmo-wijdialog").wijdialog("close");break}return this},tab:function(){var d=Array.prototype.slice.call(arguments),i="open";if(d.length>0&&a.inArray(d[0],["open","close"])!==-1){i=d.shift()}switch(i){case"open":var g=d[0]||{},f=d[1]||{},e=this.closest(".ui-dialog-content").size();if(e){this.dialog(a.extend({contentUrl:g.url,ajax:!g.iframe,title:g.label},f))}else{a.nos.tabs.add(g)}break;case"close":var h=this.closest(":wijmo-wijdialog");if(h.size()){this.dialog("close")}else{a.nos.tabs.close()}break}return this}});a.fn.nos=function(){return b(this)};return b});
