 ...
 */
define("jquery-nos",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete"],function(c){var e=void (0),d=window.$nos=c.sub(),a=e,b=function(){if(a===e){a=d(".nos-ostabs")}return a};d.extend({dispatchEvent:function(f){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.dispatchEvent(f)}b().ostabs("dispatchEvent",f)},notify:function(f,g){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.notify(f,g)}if(!c.isPlainObject(f)){f={title:f}}if(g!==e){c.extend(f,c.isPlainObject(g)?g:{type:g})}if(c.isPlainObject(f)){require(["link!static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.default.css","static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify"],function(){var h={pnotify_history:false,pnotify_addclass:"nos-notification"};c.each(f,function(i,j){if(i.substr(0,8)!=="pnotify_"){i="pnotify_"+i}h[i]=j});return c.pnotify(h)})}return false}});d.fn.extend({xhr:function(){var l=Array.prototype.slice.call(arguments),f="request";if(l.length>0&&c.inArray(l[0],["request","success","error","saveUserConfig"])!==-1){f=l.shift()}switch(f){case"request":var q=c.extend({dataType:"json",type:"POST",data:{}},l[0]||{}),h=q.success,j=q.error,p=this;if(q.dataType=="json"){q.success=function(r){if(c.isFunction(q.old_success)){r.user_success=h}p.xhr("success",r)};q.error=function(r){p.xhr("error",r);if(c.isFunction(j)){j.apply(this,arguments)}}}return c.ajax(q);break;case"success":var o=l[0];if(o.error){if(c.isArray(o.error)){c.each(o.error,function(){d.notify(this,"error")})}else{d.notify(o.error,"error")}}if(o.notify){if(c.isArray(o.notify)){c.each(o.notify,function(){d.notify(this)})}else{d.notify(o.notify)}}if(o.dispatchEvent){if(c.isArray(o.dispatchEvent)){c.each(o.dispatchEvent,function(r,s){d.dispatchEvent(s)})}else{d.dispatchEvent(o.dispatchEvent)}}if(c.isFunction(o.user_success)){o.user_success.apply(this,arguments)}var k=this.closest(".ui-dialog-content").size();if(k){if(o.closeTab){this.tab("close")}}else{if(o.redirect){document.location.href=o.redirect}if(o.replaceTab){this.tab("add",{url:o.replaceTab},false).tab("close")}}break;case"error":var m=l[0],i=l[1];if(m.status!=0){d.notify("Connection error!","error")}else{if(i=="parsererror"){d.notify("Request seemed a success, but we could not read the answer.")}else{if(i=="timeout"){d.notify("Time out (server is busy?). Please try again.")}}}break;case"saveUserConfig":var n=l[0],g=l[1];this.xhr({url:"/admin/nos/noviusos/save_user_configuration",data:{key:n,configuration:g}});break}return this},media:function(i){i=i||{};var f={all:"/admin/nos/media/list",image:"/admin/nos/media/list?view=image_pick"},j=this;var h=null;var g=c.extend({title:j.attr("title")||"File",allowDelete:true,choose:function(k){if(h===null){h=j.dialog({destroyOnClose:false,contentUrl:f[i.mode],ajax:true,title:"Choose a media file"});h.bind("select.media",function(m,l){j.inputFileThumb({file:l.thumbnail});j.val(l.id);h.wijdialog("close")})}else{h.wijdialog("open")}}},i.inputFileThumb||{});require(["static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/js/jquery.input-file-thumb","link!static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/css/jquery.input-file-thumb.css"],function(){c(function(){j.inputFileThumb(g)})})},form:function(){var h=Array.prototype.slice.call(arguments),j="ui";if(h.length>0&&c.inArray(h[0],["ui","validate","ajax"])!==-1){j=h.shift()}switch(j){case"ui":var g=this;g.find(":input[type='text'],:input[type='password'],:input[type='email'],textarea").wijtextbox();g.find(":input[type='submit'],button").each(function(){var k={};var l=c(this).data("icon");if(l){k.icons={primary:"ui-icon-"+l}}c(this).button(k)});g.find("select").filter(":not(.notransform)").nos().initOnShow("init",function(){c(this).wijdropdown()});g.find(":input[type=checkbox]").nos().initOnShow("init",function(){c(this).wijcheckbox()});g.find(".expander").each(function(){var k=c(this);k.wijexpander(c.extend({expanded:true},k.data("wijexpander-options")))});g.find(".accordion").wijaccordion({header:"h3",selectedIndexChanged:function(l,k){c(l.target).find(".ui-accordion-content").eq(k.newIndex).nos().initOnShow()}});break;case"validate":var f=this,i=h[0]||{};if(!f.is("form")){f=f.find("form")}require(["jquery-validate"],function(){f.validate(c.extend({},i,{errorClass:"ui-state-error",success:true,ignore:"input[type=hidden]"}))});break;case"ajax":var f=this;if(!f.is("form")){f=f.find("form")}require(["jquery-form"],function(){f.ajaxForm({dataType:"json",success:function(k){f.xhr("success",k).triggerHandler("ajax_success",[k])},error:function(){d.notify("An error occured","error")}})});break}return this},initOnShow:function(){var f=Array.prototype.slice.call(arguments),h="show";if(f.length>0&&c.inArray(f[0],["init","show"])!==-1){h=f.shift()}switch(h){case"init":var g=f[0];if(!c.isFunction(g)){return}this.each(function(){var i=c(this);if(i.is(":visible")){g.call(i)}else{i.addClass("nos-init-on-show").data("nos-init-on-show",g)}});break;case"show":this.find(".nos-init-on-show:visible").each(function(){var i=c(this),j=i.data("nos-init-on-show");i.removeClass("nos-init-on-show");j.call(this)});break}return this},dialog:function(){var l=Array.prototype.slice.call(arguments),f="open";if(l.length>0&&c.inArray(l[0],["open","close"])!==-1){f=l.shift()}switch(f){case"open":var o=l[0]||{},p=c.extend(true,{},{destroyOnClose:true,width:window.innerWidth-200,height:window.innerHeight-100,modal:true,captionButtons:{pin:{visible:false},refresh:{visible:o.contentUrl!=null&&!o.ajax},toggle:{visible:false},minimize:{visible:false},maximize:{visible:false}}},o),g=p.close,j=p.open,m=this.closest(".nos-dispatcher, body"),n=this,h=c("<div></div>").appendTo(m);p.close=function(){if(c.isFunction(g)){g.apply(h,arguments)}if(p.destroyOnClose){h.wijdialog("destroy").remove()}else{h.closest(".ui-dialog").hide().appendTo(m)}};if(!p.destroyOnClose){p.open=function(){if(c.isFunction(j)){j.apply(h,arguments)}h.closest(".ui-dialog").appendTo("body")}}if(p.content!==e){h.append(p.content)}var k=true;if(p.ajax){var i=p.contentUrl;delete p.contentUrl;p.autoOpen=false;h.wijdialog(p);c.ajax({url:i,type:"GET",dataType:"html",complete:function(s,q,t){t=s.responseText;if(s.isResolved()){s.done(function(v){t=v});try{var r=c.parseJSON(t);k=false}catch(u){}if(k){h.wijdialog("open").html(t)}else{h.empty().wijdialog("destroy").remove();n.xhr("success",r)}}}})}else{h.wijdialog(p)}if(k&&c.isFunction(p.onLoad)){p.onLoad()}return h;break;case"close":this.closest(":wijmo-wijdialog").wijdialog("close");break}return this},tab:function(){var p=Array.prototype.slice.call(arguments),f="open";if(p.length>0&&c.inArray(p[0],["open","close","add","update","init"])!==-1){f=p.shift()}var g=function(s){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).data("nos-ostabs-index")}if(s==="current"){return b().ostabs("current").index}if(c.isNumeric(s)){return s}var t=c(s).parents(".nos-ostabs-panel-content");if(t.size()){return t.data("nos-ostabs-index")}return false};switch(f){case"open":var k=p[0]||{},l=p[1]||{},o=this.closest(".ui-dialog-content").size();if(o){this.dialog(c.extend({contentUrl:k.url,ajax:!k.iframe,title:k.label},l))}else{if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).tab("open",k,l)}if(b().length){var r=b().ostabs("tabs"),h=false;c.each(r,function(u,s){if(s.url===k.url){h=u;return false}});if(h!==false){return b().ostabs("select",h+4)}else{this.tab("add",k)}}else{if(k.url){window.open(k.url)}}}break;case"add":var k=p[0],m=p[1];if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).tab("add",k,m)}var n;if(m!==e&&m!==true){n=g("current")+1}if(b().length){n=b().ostabs("add",k,n);b().ostabs("select",n)}else{if(k.url){window.open(k.url)}}break;case"close":var j=this.closest(":wijmo-wijdialog");if(j.size()){this.dialog("close")}else{if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).tab("close")}var n=g(this);if(b().length){b().ostabs("remove",n)}}break;case"update":var k=p[0];if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).tab("update",k)}if(this.size()&&!this.closest(".ui-dialog-content").size()){if(b().length){var n=g(this);b().ostabs("update",n,k)}}break;case"init":var i=p[0],q=function(s){if(b().length){b().xhr("saveUserConfig","tabs",{selected:a.ostabs("option","selected"),tabs:a.ostabs("tabs")})}};a=this;c.extend(i,{add:q,remove:q,select:q,show:q,drag:q});if(i.user_configuration["tabs"]){if(!i.options){i.options={}}i.initTabs=i.user_configuration["tabs"]["tabs"];i.selected=i.user_configuration["tabs"]["selected"]}a.ostabs(i);break}return this}});c.fn.nos=function(){return d(this)};return d});
