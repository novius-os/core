...
define("jquery-nos-ostabs",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete","order!jquery-nos","order!jquery-nos-loadspinner"],function(g,f,e,k,j,i){var h=void (0);g.widget("nos.ostabs",{options:{initTabs:[],newTab:"appsTab",trayTabs:[],appsTab:{},fx:null,labelMinWidth:100,labelMaxWidth:200,selected:null,texts:{scrollLeft:"Scroll left",scrollRight:"Scroll right",newTab:"New tab",removeTab:"Remove tab",closeTab:"Close tab",reloadTab:"Reload tab",spinner:"Loading..."},add:null,remove:null,select:null,show:null,drag:null},tabId:0,openRank:0,_create:function(){var a=this,b=a.options;a._tabify(true);g(window).resize(function(){if(b.selected){a._firePanelEvent(a.panels.eq(b.selected),g.Event("resizePanel"))}})},_setOption:function(b,c){var a=this;if(b=="selected"){a.select(c)}else{a.options[b]=c;a._tabify()}},_getNextTabId:function(){var a=this;return ++a.tabId},_tabId:function(b){var c=this;return g(b).data("panelid.tabs")||"nos-ostabs-"+c._getNextTabId()},_sanitizeSelector:function(a){return a.replace(/:/g,"\\:")},_ui:function(a,b){var c=this,d=c.lis.index(a);if(b===h){b=c.element.find(c._sanitizeSelector(c.anchors[d].hash))[0]}return{tab:a,panel:b,index:d}},_cleanup:function(){var a=this;a.lis.filter(".ui-state-processing").removeClass("ui-state-processing").find("span.nos-ostabs-icon").removeClass("nos-ostabs-loader").each(function(){if(g.isFunction(g.fn.loadspinner)){g(this).loadspinner("destroy")}}).html(" ").parent().find("span.nos-ostabs-label").each(function(){var c=g(this),b=c.data("label.tabs");if(b){c.html(b).removeData("label.tabs")}});return a},_tabify:function(u){var v=this,d=v.options,b=/^#.+/;if(u){v.element.addClass("nos-ostabs ui-widget ui-widget-content");v.uiOstabsHeader=g("<div></div>").addClass("ui-widget-header").appendTo(v.element);v.uiOstabsSuperPanel=g("<div></div>").addClass("nos-ostabs-tabs").appendTo(v.uiOstabsHeader);v.uiOstabsTray=g("<ul></ul>").addClass("nos-ostabs-tray nos-ostabs-nav");if(g.isArray(d.trayTabs)){g.each(d.trayTabs,function(o,w){if(g.isPlainObject(w)){v._add(w,v.uiOstabsTray).addClass("nos-ostabs-tray")}})}v.uiOstabsTray.prependTo(v.uiOstabsHeader);if(g.isPlainObject(d.appsTab)){v.uiOstabsAppsTab=g("<ul></ul>").addClass("nos-ostabs-appstab  nos-ostabs-nav").prependTo(v.uiOstabsHeader);v._add(d.appsTab,v.uiOstabsAppsTab).addClass("nos-ostabs-appstab").removeClass("ui-state-default");v.uiOstabsAppsTab.find("> li > a:first").click(function(){i().refreshNosPanel()})}else{v.uiOstabsAppsTab=g("<ul></ul>")}v.uiOstabsSuperPanel.css("left",v.uiOstabsAppsTab.outerWidth(true)+25).css("right",v.uiOstabsTray.outerWidth(true)+35).wijsuperpanel({allowResize:false,autoRefresh:true,hScroller:{scrollBarVisibility:"hidden",scrollMode:"buttons",increaseButtonPosition:{my:"left bottom",at:"right bottom"},decreaseButtonPosition:{my:"right bottom",at:"left bottom"}},vScroller:{scrollBarVisibility:"hidden"},showRounder:false}).removeClass("ui-widget ui-widget-content").css("overflow","visible");v.uiOstabsSuperPanelContent=v.uiOstabsSuperPanel.wijsuperpanel("getContentElement");v.uiOstabsTabs=g("<ul></ul>").appendTo(v.uiOstabsSuperPanelContent);if(g.isArray(d.initTabs)){g.each(d.initTabs,function(o,w){v._add(w)})}var c=d.newTab;if(g.isPlainObject(c)){c=g.extend({label:d.texts.newTab,iconClasses:"ui-icon ui-icon-circle-plus"},c)}else{if(c&&g.isPlainObject(d.appsTab)){c=g.extend({},d.appsTab,{label:d.texts.newTab,iconClasses:"ui-icon ui-icon-circle-plus",iconUrl:"",iconSize:16})}else{c=false}}if(c){v.uiOstabsNewTab=v._add(c).addClass("nos-ostabs-newtab")}else{v.uiOstabsNewTab=g("<li>/<li>")}v.uiOstabsNewTab.hover(function(){g(this).find(".nos-ostabs-label").css("display","inline-block");v._tabsWidth()},function(){g(this).find(".nos-ostabs-label").css("display","none");v._tabsWidth()});v.uiOstabsTabs.sortable({items:"li:not(.nos-ostabs-newtab)",appendTo:v.uiOstabsSuperPanelContent,cursor:"move",delay:250,scroll:false,helper:"clone",tolerance:"pointer",axis:"x",zIndex:100000,placeholder:"ui-state-highlight",forcePlaceholderSize:true,start:function(){v.sorting=true},stop:function(){v.sorting=false},update:function(){v._trigger("drag",null);v.lis=v.uiOstabsAppsTab.add(v.uiOstabsTray).add(v.uiOstabsTabs).find("li:has(a[href])");v.anchors=v.lis.map(function(w){var o=g("a",this)[0];v.element.find(v._sanitizeSelector(o.hash)).find(".nos-ostabs-panel-content").data("nos-ostabs-index",w);return o});v._tabsWidth()}});v.tabsWidth=v.uiOstabsSuperPanelContent.width();v.labelWidth=d.labelMaxWidth;v.uiOstabsTabs.width(v.tabsWidth)}var r=[];v.lis=v.uiOstabsAppsTab.add(v.uiOstabsTray).add(v.uiOstabsTabs).find("li:has(a[href])").each(function(o){var x=g(this),w=x.data("ui-ostab")||{};if(w.openRank!==false){r[w.openRank]=x}});v.openRank=1;g.each(r,function(o,x){if(x){var w=x.data("ui-ostab")||{};w.openRank=v.openRank++}});v.anchors=v.lis.map(function(){return g("a",this)[0]});v.panels=g([]);v.anchors.each(function(x,o){var w=g(o).attr("href");var z=w.split("#")[0],A,B;if(z&&(z===location.toString().split("#")[0]||(A=g("base")[0])&&z===A.href)){w=o.hash;o.href=w}if(b.test(w)){B=v.element.find(v._sanitizeSelector(w));v.panels=v.panels.add(B)}else{if(w&&w!=="#"){g.data(o,"href.tabs",w);g.data(o,"load.tabs",w.replace(/#.*$/,""));var C=v._tabId(o);o.href="#"+C;B=v.element.find("#"+C);if(!B.length){var y=v.lis.eq(x).data("ui-ostab");B=g("<div></div>").attr("id",C).addClass("nos-ostabs-panel ui-widget-content ui-corner-bottom nos-ostabs-hide").appendTo(v.element);v._actions(B,x,y.actions||[])}v.panels=v.panels.add(B)}}B.find(".nos-ostabs-panel-content").data("nos-ostabs-index",x)});if(u){v.uiOstabsTray.add(v.uiOstabsAppsTab).addClass("nos-ostabs-nav ui-helper-reset ui-helper-clearfix");v.lis.addClass("ui-corner-top");v.panels.addClass("nos-ostabs-panel ui-widget-content");if(d.selected===h){if(location.hash){v.anchors.each(function(w,o){if(o.hash==location.hash){d.selected=w;return false}})}if(typeof d.selected!=="number"&&v.lis.filter(".nos-ostabs-selected").length){d.selected=v.lis.index(v.lis.filter(".nos-ostabs-selected"))}d.selected=d.selected||(v.lis.length?0:-1)}d.selected=((d.selected>=0&&v.anchors[d.selected])||d.selected<0)?d.selected:0;v.panels.addClass("nos-ostabs-hide");v.lis.removeClass("nos-ostabs-selected ui-state-active");if(d.selected>=0&&v.anchors.length){v.element.find(v._sanitizeSelector(v.anchors[d.selected].hash)).removeClass("nos-ostabs-hide");v.lis.eq(d.selected).addClass("nos-ostabs-selected ui-state-active ui-state-open");v.element.queue("tabs",function(){v._trigger("show",null,v._ui(v.lis[d.selected]))});v.title(d.selected,v.title(d.selected));v._load(d.selected)}g(window).bind("unload",function(){v.lis.add(v.anchors).unbind(".tabs");v.lis=v.anchors=v.panels=null})}else{d.selected=v.lis.index(v.lis.filter(".nos-ostabs-selected"))}v.anchors.removeData("cache.tabs");v.lis.add(v.anchors).unbind(".tabs");var m=function(w,o){o.addClass("ui-state-"+w)};var q=function(w,o){o.removeClass("ui-state-"+w)};v.lis.bind("mouseover.tabs",function(){m("hover",g(this))});v.lis.bind("mouseout.tabs",function(){q("hover",g(this))});v.anchors.bind("focus.tabs",function(){m("focus",g(this).closest("li"))});v.anchors.bind("blur.tabs",function(){q("focus",g(this).closest("li"))});var a,n;if(d.fx){if(g.isArray(d.fx)){a=d.fx[0];n=d.fx[1]}else{a=n=d.fx}}function l(o,w){o.css("display","");if(!g.support.opacity&&w.opacity){o[0].style.removeAttribute("filter")}}function p(x){var w=x.data("callbacks.ostabs"),o=x.find(".nos-dispatcher");if(g.isPlainObject(w)){g.each(w,function(y,z){v._firePanelEvent(o,z)});w={}}}var s=n?function(o,w){var x=g(o).closest("li").addClass("nos-ostabs-selected ui-state-active");w.hide().removeClass("nos-ostabs-hide").animate(n,n.duration||"normal",function(){if(x.hasClass("nos-ostabs-newtab")){v._tabsWidth()}v.uiOstabsSuperPanel.wijsuperpanel("scrollChildIntoView",x.find("a"));l(w,n);p(w);v._firePanelEvent(w,g.Event("showPanel"))})}:function(o,w){var x=g(o).closest("li");if(x.hasClass("nos-ostabs-newtab")){v._tabsWidth()}v.uiOstabsSuperPanel.wijsuperpanel("scrollChildIntoView",x.find("a"));x.addClass("nos-ostabs-selected ui-state-active");w.removeClass("nos-ostabs-hide");p(w);v._firePanelEvent(w,g.Event("showPanel"))};var t=a?function(w,o){o.animate(a,a.duration||"normal",function(){if(v.uiOstabsNewTab.hasClass("ui-state-active")){v._tabsWidth()}v.lis.removeClass("nos-ostabs-selected ui-state-active");o.addClass("nos-ostabs-hide");l(o,a);v.element.dequeue("tabs");o.trigger("hidePanel.ostabs")})}:function(w,o){if(v.uiOstabsNewTab.hasClass("ui-state-active")){v._tabsWidth()}v.lis.removeClass("nos-ostabs-selected ui-state-active");o.addClass("nos-ostabs-hide");v.element.dequeue("tabs");o.trigger("hidePanel.ostabs")};v.anchors.bind("click.tabs",function(){var x=this,z=g(x).closest("li"),o=v.panels.filter(":not(.nos-ostabs-hide)"),y=v.element.find(v._sanitizeSelector(x.hash)),w=z.data("ui-ostab");z.addClass("ui-state-open");v.uiOstabsNewTab.removeClass("ui-state-open");d.selected=v.anchors.index(this);if(z.hasClass("nos-ostabs-selected")||z.hasClass("ui-state-processing")||v.panels.filter(":animated").length||v._trigger("select",null,v._ui(z[0],y[0]))===false){this.blur();return false}v._abort();if(y.length){if(o.length){v.element.queue("tabs",function(){t(x,o)})}v.element.queue("tabs",function(){w.openRank=v.openRank++;s(x,y)});g("title").text(z.find(".nos-ostabs-label").text());v._load(v.anchors.index(this))}else{throw"jQuery UI Tabs: Mismatching fragment identifier."}this.blur()});v.anchors.bind("click.tabs",function(){return false});v._tabsWidth()},_getIndex:function(b){var a=this;if(typeof b=="string"){b=a.anchors.index(a.anchors.filter("[href$="+b+"]"))}return b},_actions:function(w,n,q){var v=this,d=v.options;var r=v.lis.eq(n),p=v.anchors.eq(n);w.find(".nos-ostabs-actions").remove();var l=g("<div></div>").addClass("nos-ostabs-actions ui-state-active").prependTo(w);var t=g("<div></div>").addClass("nos-ostabs-actions-links").prependTo(l);var c=r.not(".nos-ostabs-tray").not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length;var s=r.not(".nos-ostabs-appstab").length;var m=p.data("iframe.tabs");if(s){var u=g('<a href="#"></a>').addClass("nos-ostabs-close").click(function(){v.remove(v.lis.index(r));return false}).appendTo(t);g("<span></span>").addClass("ui-icon ui-icon-closethick").text(c?d.texts.removeTab:d.texts.closeTab).appendTo(u);g("<span></span>").text(c?d.texts.removeTab:d.texts.closeTab).appendTo(u)}if(m){var b=g('<a href="#"></a>').addClass("nos-ostabs-reload").click(function(){var a=w.find("iframe.nos-ostabs-panel-content");if(a!==h){a.attr("src",a.attr("src"))}return false}).text(d.texts.reloadTab).appendTo(t);g("<span></span>").addClass("ui-icon ui-icon-refresh").text(d.texts.reloadTab).appendTo(b)}g.each(q,function(){var x=this;var a=g('<a href="#"></a>').addClass("nos-ostabs-action").click(function(y){y.preventDefault();if(g.isFunction(x.click)){x.click(y,{ui:a,action:x})}}).appendTo(t);var o=g("<span></span>").addClass("ui-icon").text(x.label||"").appendTo(a);if(x.iconUrl){o.css("background-image",'url("'+x.iconUrl+'") !important')}else{o.addClass(x.iconClasses)}g("<span></span>").text(x.label||"").appendTo(a)})},_tabsWidth:function(){var a=this,l=a.options,c=0;a.uiOstabsTabs.width(10000000);a.uiOstabsTabs.find("li").each(function(){c+=g(this).outerWidth(true)});a.uiOstabsTabs.width(c);var b=a.uiOstabsTabs.find(".nos-ostabs-label:visible").length,d;if(a.tabsWidth<a.uiOstabsTabs.width()){while(a.tabsWidth<a.uiOstabsTabs.width()&&a.labelWidth>l.labelMinWidth){d=a.labelWidth-l.labelMinWidth;d=d>10?10:d;c=c-b*d;a.uiOstabsTabs.width(c);a.labelWidth=a.labelWidth-d;g("head .tabswidth").remove();g('<style type="text/css" class="tabswidth">.nos-ostabs .ui-widget-header .nos-ostabs-label {width : '+a.labelWidth+"px !important;}</style>").appendTo("head")}}else{do{d=l.labelMaxWidth-a.labelWidth;d=d>10?10:d;if(a.tabsWidth>(c+b*10)){c=c+b*d;a.uiOstabsTabs.width(c);a.labelWidth=a.labelWidth+d;g("head .tabswidth").remove();g('<style type="text/css" class="tabswidth">.nos-ostabs .ui-widget-header .nos-ostabs-label {width : '+a.labelWidth+"px !important;}</style>").appendTo("head")}}while(a.tabsWidth>(c+b*10)&&a.labelWidth<l.labelMaxWidth)}if(a.tabsWidth<a.uiOstabsTabs.width()){a.uiOstabsSuperPanel.find(".wijmo-wijsuperpanel-buttonleft, .wijmo-wijsuperpanel-buttonright").show()}else{a.uiOstabsSuperPanel.find(".wijmo-wijsuperpanel-buttonleft, .wijmo-wijsuperpanel-buttonright").hide()}},add:function(c,b){var a=this;if(!g.isPlainObject(c)||c.url===h){return false}if(b===h){b=a.anchors.length-1}var d=a._add(c);if(b<a.lis.length){d.insertBefore(a.lis[b])}else{b=a.lis.eq(d)}a.uiOstabsTabs.sortable("refresh");a._tabify();if(a.anchors.length==1){a.select(0)}a._trigger("add",c,a._ui(d[0]));return b},_add:function(n,o){var d=this;if(!g.isPlainObject(n)||n.url===h){return false}o=o||d.uiOstabsTabs;n=g.extend({url:"",iframe:false,label:"",labelDisplay:true,iconClasses:"ui-icon ui-icon-document",iconUrl:"",openRank:false,iconSize:16,panelId:false,actions:[]},n);if(n.openRank!==false&&n.openRank>=d.openRank){d.openRank=n.openRank+1}var c=g('<a href="'+n.url+'"></a>');if(n.iframe){c.data("iframe.tabs",true)}if(n.panelId){c.data("panelid.tabs",n.panelId)}var m=d._icon(n).appendTo(c);var l=g("<span></span>").addClass("nos-ostabs-label").text(n.label?n.label:"New tab").appendTo(c);if(!n.labelDisplay){l.hide()}var b=g("<li></li>").append(c).attr("title",n.label).addClass("ui-corner-top ui-state-default").data("ui-ostab",n).appendTo(o);if(!isNaN(n.iconSize)&&n.iconSize!==16&&o!==d.uiOstabsTabs){b.css({height:(n.iconSize+4)+"px",bottom:(n.iconSize-35)+"px"});m.css("top","2px")}return b},remove:function(c){var b=this;c=b._getIndex(c);var o=b.lis.eq(c),m=b.anchors.eq(c),n=b.element.find(b._sanitizeSelector(b.anchors[c].hash)),a=0,d=0;if(c==0&&!o.hasClass("nos-ostabs-selected")){var l=b.lis.filter(".nos-ostabs-newtab");if(l.hasClass("nos-ostabs-selected")){o=l}}if(o.not(".nos-ostabs-tray").not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length){o.remove();n.remove()}o.removeClass("ui-state-active ui-state-open");b.lis.each(function(q){var p=g(this),r=p.data("ui-ostab")||{};if(p[0]!=o[0]&&r.openRank&&r.openRank>d){d=r.openRank;a=q}});b.select(a);if(o.not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length){g("> *",n).not(".nos-ostabs-actions").remove()}n.addClass("nos-ostabs-hide").removeData("callbacks.ostabs");o.removeClass("nos-ostabs-selected");b._tabify();b._trigger("remove",null,b._ui(o[0],n[0]));return b},title:function(b,d){var a=this,c=a.options;b=a._getIndex(b);var l=a.lis.eq(b);if(d===h){return l.find(".nos-ostabs-label").text()}else{l.find(".nos-ostabs-label").text(d);if(c.selected==b){g("title").text(d)}a._trigger("title",null,a._ui(l[0]));return a}},update:function(d,l){var c=this;d=c._getIndex(d);if(!g.isPlainObject(l)){return false}var a=!!l.url&&l.reload,o=c.lis.eq(d);l=g.extend({},o.data("ui-ostab"),l);if(a){delete l.reload;this.remove(d);this.add(l,d);this.select(d)}else{if(c.options.selected==d){g("title").text(l.label)}var b=c._add(l),m=b.find("a"),n=c.panels.eq(d);o.data("ui-ostab",l).attr("title",l.label||"").addClass(b.attr("class")).css({height:b.css("height"),bottom:b.css("bottom")}).find("a").empty().append(m.children());c._actions(n,d,l.actions||[]);b.remove();c._trigger("update",null,c._ui(o[0],n[0]))}return c},_icon:function(b){var a=g("<span></span>").addClass("nos-ostabs-icon");if(b.iconUrl){a.css("background-image",'url("'+b.iconUrl+'")')}else{a.addClass(b.iconClasses)}if(!isNaN(b.iconSize)&&b.iconSize!==16){a.css({width:b.iconSize+"px",height:b.iconSize+"px",lineHeight:b.iconSize+"px",top:(b.iconSize>22?22-b.iconSize:10-b.iconSize/2)+"px"})}return a},select:function(b){var a=this;b=a._getIndex(b);if(b==-1){return a}a.anchors.eq(b).trigger("click.tabs");return a},_load:function(m){var d=this;m=d._getIndex(m);var q=d.options,c=d.anchors.eq(m)[0],l=g.data(c,"load.tabs"),n=g.data(c,"iframe.tabs");d._abort();if((!l&&n)||d.element.queue("tabs").length!==0&&g.data(c,"cache.tabs")){d.element.dequeue("tabs");return}var b=d.element.find(d._sanitizeSelector(c.hash)),p=g("> *",b).not(".nos-ostabs-actions").length;if(p!==0){d.element.dequeue("tabs");return}d.lis.eq(m).addClass("ui-state-processing");g("span.nos-ostabs-label",c).each(function(){var a=g(this);a.data("label.tabs",a.html()).html(a.data("label.tabs")?q.texts.spinner:"")});if(g.isFunction(g.fn.loadspinner)){g("span.nos-ostabs-icon",c).each(function(){var a=g(this);a.addClass("ui-state-processing").loadspinner({diameter:a.width(),scaling:true})})}if(!n){d.xhr=g.ajax({url:l,success:function(a){g("<div></div>").addClass("nos-ostabs-panel-content nos-dispatcher").data("nos-ostabs-index",m).prependTo(b.data("callbacks.ostabs",{})).html(a);g.data(c,"cache.tabs",true)},complete:function(){d._cleanup();d._trigger("load",null,d._ui(d.lis[m]))}})}else{g("<iframe "+(g.browser.msie?'allowTransparency="true" ':"")+'src="'+l+'" frameborder="0"></iframe>').data("nos-ostabs-index",m).addClass("nos-ostabs-panel-content nos-dispatcher").bind("load",function(){d._cleanup();d._trigger("load",null,d._ui(d.lis[m]))}).prependTo(b.data("callbacks.ostabs",{}));g.data(c,"cache.tabs",true)}d.element.dequeue("tabs");return d},_abort:function(){var a=this;a.element.queue([]);a.panels.stop(false,true);a.element.queue("tabs",a.element.queue("tabs").splice(-2,2));a._cleanup();return a},tabs:function(){var a=this,b=[];a.uiOstabsTabs.find("li:not(.nos-ostabs-newtab)").each(function(){var c=g.extend({},g(this).data("ui-ostab"));delete c.actions;b.push(c)});return b},dispatchEvent:function(b){var a=this,c=a.options;g.each(a.panels,function(d){var m=g(this);if(d===c.selected){a._firePanelEvent(m,b)}else{var l=m.data("callbacks.ostabs");if(g.isPlainObject(l)){l[b.type+(b.namespace?"."+b.namespace:"")]=b}}});return a},_firePanelEvent:function(a,b){a=a.is(".nos-dispatcher")?a:a.find(".nos-dispatcher");if(a.is("iframe")){if(a[0].contentDocument.$){a[0].contentDocument.$("body").trigger(b)}}else{try{a.trigger(b)}catch(c){log("_firePanelEvent error",c,b)}}},current:function(){var a=this,b=a.options;return{tab:g(a.lis.get(b.selected)),panel:g(a.panels.get(b.selected)),index:b.selected}}});return i});
