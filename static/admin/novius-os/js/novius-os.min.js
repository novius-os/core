 ...
 */
define("jquery-nos",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete"],function(a){var b=void (0);a.nos={dispatchEvent:function(c){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.dispatchEvent(c)}if(a.nos.$noviusos){a.nos.$noviusos.ostabs("dispatchEvent",c)}},dataStore:{},data:function(d,c){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.data(d,c)}if(d){if(c){this.dataStore[d]=c}return this.dataStore[d]}},dialog:function(d){if(d.destroyOnClose){var e=d.close;d.close=function(){if(a.isFunction(e)){e.apply(this,arguments)}a(this).wijdialog("destroy").remove()}}d=a.extend(true,{},{width:window.innerWidth-200,height:window.innerHeight-100,modal:true,captionButtons:{pin:{visible:false},refresh:{visible:d.contentUrl!=null&&!d.ajax},toggle:{visible:false},minimize:{visible:false},maximize:{visible:false}}},d);var c=a.nos.$noviusos.ostabs?a.nos.$noviusos.ostabs("current").panel:a("body");var g=a(document.createElement("div")).appendTo(c);a.nos.data("dialog",g);if(typeof d.content!="undefined"){g.append(d.content)}var f=true;if(d.ajax){var h=d.contentUrl;delete d.contentUrl;d.autoOpen=false;g.wijdialog(d);a.ajax({url:h,type:"GET",dataType:"html",complete:function(k,i,l){l=k.responseText;if(k.isResolved()){k.done(function(n){l=n});try{var j=a.parseJSON(l);f=false}catch(m){}if(f){g.wijdialog("open")}else{g.empty();g.wijdialog("destroy");g.remove();a.nos.ajax.success(j)}g.html(l)}}})}else{g.wijdialog(d)}if(f){if(a.isFunction(d.onLoad)){d.onLoad()}g.bind("wijdialogclose",function(i,j){g.closest(".ui-dialog").hide().appendTo(c)})}return g},notify:function(c,d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.notify(c,d)}if(!a.isPlainObject(c)){c={title:c}}if(d!==b){a.extend(c,a.isPlainObject(d)?d:{type:d})}if(a.isPlainObject(c)){require(["link!static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.default.css","static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.min"],function(){var e={pnotify_history:false,pnotify_addclass:"nos-notification"};a.each(c,function(f,g){if(f.substr(0,8)!=="pnotify_"){f="pnotify_"+f}e[f]=g});return a.pnotify(e)})}return false},ajax:{request:function(c){c=a.extend({dataType:"json",type:"POST",data:{}},c);if(c.dataType=="json"){if(a.isFunction(c.success)){var e=c.success;c.success=function(f){f.user_success=e;a.nos.ajax.success(f)}}else{c.success=a.nos.ajax.success}if(a.isFunction(c.error)){var d=c.error;c.error=function(f){a.nos.ajax.error(f);d.apply(this,arguments)}}else{c.error=a.nos.ajax.error}}a.ajax(c)},success:function(c){if(c.error){if(a.isArray(c.error)){a.each(c.error,function(){a.nos.notify(this,"error")})}else{a.nos.notify(c.error,"error")}}if(c.notify){if(a.isArray(c.notify)){a.each(c.notify,function(){a.nos.notify(this)})}else{a.nos.notify(c.notify)}}if(c.dispatchEvent){if(a.isArray(c.dispatchEvent)){a.each(c.dispatchEvent,function(d,e){a.nos.dispatchEvent(e)})}else{a.nos.dispatchEvent(c.dispatchEvent)}}if(a.isFunction(c.user_success)){c.user_success.apply(this,arguments)}if(c.redirect){document.location=c.redirect}if(c.closeTab){a.nos.tabs.close()}if(c.replaceTab){a.nos.tabs.replace(c.replaceTab)}},error:function(c,d){if(c.status!=0){a.nos.notify("Connection error!","error")}else{if(d=="parsererror"){a.nos.notify("Request seemed a success, but we could not read the answer.")}else{if(d=="timeout"){a.nos.notify("Time out (server is busy?). Please try again.")}}}}},media:function(d,g){var c={all:"/admin/nos/media/list",image:"/admin/nos/media/list?view=image_pick"};var f=null;var e=a.extend({title:d.attr("title")||"File",allowDelete:true,choose:function(h){if(f==null){f=a.nos.dialog({contentUrl:c[g.mode],ajax:true,title:"Choose a media file"});f.bind("select.media",function(j,i){d.inputFileThumb({file:i.thumbnail});d.val(i.id);f.wijdialog("close")})}else{f.wijdialog("open")}}},g.inputFileThumb);require(["static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/js/jquery.input-file-thumb","link!static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/css/jquery.input-file-thumb.css"],function(){a(function(){d.inputFileThumb(e)})})},tabOrDialog:{open:function(e,f,d){var c=a(e).closest(".ui-dialog-content").size();if(c){d=d||{};a.nos.dialog(a.extend({contentUrl:f.url,ajax:!f.iframe,title:f.label},d))}else{a.nos.tabs.add(f)}},close:function(c){var d=a(c).closest(":wijmo-wijdialog");if(d.size()){d.wijdialog("close").wijdialog("destroy").remove()}else{a.nos.tabs.close()}}},form:{ajax:function(c){if(!c){return}var d=a(c);if(!d.is("form")){d=d.find("form")}a.nos.ui.validate(d,{})}},ui:{form:function(c){c=c||"body";a(function(){var d=a(c);d.find(":input[type='text'],:input[type='password'],:input[type='email'],textarea").wijtextbox();d.find(":input[type='submit'],button").each(function(){var e={};var f=a(this).data("icon");if(f){e.icons={primary:"ui-icon-"+f}}a(this).button(e)});d.find("select").filter(":not(.notransform)").wijdropdown();d.find(":input[type=checkbox]").wijcheckbox();d.find(".expander").each(function(){var e=a(this);e.wijexpander(a.extend({expanded:true},e.data("wijexpander-options")))});d.find(".accordion").wijaccordion({header:"h3",selectedIndexChanged:function(g,f){a.nos.ui.initOnShow.show(a(g.target).find(".ui-accordion-content").eq(f.newIndex))}})})},initOnShow:{init:function(c,d){if(!a.isFunction(d)){return}if(c.is(":visible")){d()}else{c.addClass("nos-init-on-show").data("nos-init-on-show",d)}},show:function(c){c.find(".nos-init-on-show:visible").each(function(){var d=a(this),e=d.data("nos-init-on-show");d.removeClass("nos-init-on-show");e()})}},validate:function(d,e){var c=function(f){require(["jquery-form"],function(){a(f).ajaxSubmit({dataType:"json",success:function(g){a.nos.ajax.success(g);a(f).triggerHandler("ajax_success",[g])},error:function(){a.nos.notify("An error occured","error")}})})};a(function(){if(!e||a.isEmptyObject(e)){a(d).submit(function(f){c(d);f.preventDefault()})}else{require(["jquery-validate"],function(){a(d).validate(a.extend({},e,{errorClass:"ui-state-error",success:true,ignore:"input[type=hidden]",submitHandler:c}))})}})}},saveUserConfiguration:function(c,d){this.ajax.request({url:"/admin/nos/noviusos/save_user_configuration",data:{key:c,configuration:d}})}};window.$nos=a;return a});
