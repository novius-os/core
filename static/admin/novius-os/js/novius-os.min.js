 ...
 */
define("jquery-nos",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete"],function(a){var c=void (0);a.nos={dispatchEvent:function(d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.dispatchEvent(d)}if(a.nos.$noviusos){a.nos.$noviusos.ostabs("dispatchEvent",d)}},notify:function(d,e){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.notify(d,e)}if(!a.isPlainObject(d)){d={title:d}}if(e!==c){a.extend(d,a.isPlainObject(e)?e:{type:e})}if(a.isPlainObject(d)){require(["link!static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.default.css","static/novius-os/admin/vendor/jquery/pnotify/jquery.pnotify.min"],function(){var f={pnotify_history:false,pnotify_addclass:"nos-notification"};a.each(d,function(g,h){if(g.substr(0,8)!=="pnotify_"){g="pnotify_"+g}f[g]=h});return a.pnotify(f)})}return false},ajax:{request:function(d){d=a.extend({dataType:"json",type:"POST",data:{}},d);if(d.dataType=="json"){if(a.isFunction(d.success)){var f=d.success;d.success=function(g){g.user_success=f;a.nos.ajax.success(g)}}else{d.success=a.nos.ajax.success}if(a.isFunction(d.error)){var e=d.error;d.error=function(g){a.nos.ajax.error(g);e.apply(this,arguments)}}else{d.error=a.nos.ajax.error}}a.ajax(d)},success:function(d){if(d.error){if(a.isArray(d.error)){a.each(d.error,function(){a.nos.notify(this,"error")})}else{a.nos.notify(d.error,"error")}}if(d.notify){if(a.isArray(d.notify)){a.each(d.notify,function(){a.nos.notify(this)})}else{a.nos.notify(d.notify)}}if(d.dispatchEvent){if(a.isArray(d.dispatchEvent)){a.each(d.dispatchEvent,function(e,f){a.nos.dispatchEvent(f)})}else{a.nos.dispatchEvent(d.dispatchEvent)}}if(a.isFunction(d.user_success)){d.user_success.apply(this,arguments)}if(d.redirect){document.location=d.redirect}if(d.closeTab){a.nos.tabs.close()}if(d.replaceTab){a.nos.tabs.replace(d.replaceTab)}},error:function(d,f){if(d.status!=0){a.nos.notify("Connection error!","error")}else{if(f=="parsererror"){a.nos.notify("Request seemed a success, but we could not read the answer.")}else{if(f=="timeout"){a.nos.notify("Time out (server is busy?). Please try again.")}}}}},saveUserConfiguration:function(d,e){this.ajax.request({url:"/admin/nos/noviusos/save_user_configuration",data:{key:d,configuration:e}})}};var b=window.$nos=a.sub();b.fn.extend({media:function(g){g=g||{};var d={all:"/admin/nos/media/list",image:"/admin/nos/media/list?view=image_pick"},h=this;var f=null;var e=a.extend({title:h.attr("title")||"File",allowDelete:true,choose:function(i){if(f==null){f=h.nos().dialog({contentUrl:d[g.mode],ajax:true,title:"Choose a media file"});f.bind("select.media",function(k,j){h.inputFileThumb({file:j.thumbnail});h.val(j.id);f.wijdialog("close")})}else{f.wijdialog("open")}}},g.inputFileThumb||{});require(["static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/js/jquery.input-file-thumb","link!static/novius-os/admin/vendor/jquery/jquery-ui-input-file-thumb/css/jquery.input-file-thumb.css"],function(){a(function(){h.inputFileThumb(e)})})},form:function(){var f=Array.prototype.slice.call(arguments),h="ui";if(f.length>0&&a.inArray(f[0],["ui","validate","ajax"])!==-1){h=f.shift()}switch(h){case"ui":var e=this;e.find(":input[type='text'],:input[type='password'],:input[type='email'],textarea").wijtextbox();e.find(":input[type='submit'],button").each(function(){var i={};var j=a(this).data("icon");if(j){i.icons={primary:"ui-icon-"+j}}a(this).button(i)});e.find("select").filter(":not(.notransform)").nos().initOnShow("init",function(){a(this).wijdropdown()});e.find(":input[type=checkbox]").nos().initOnShow("init",function(){a(this).wijcheckbox()});e.find(".expander").each(function(){var i=a(this);i.wijexpander(a.extend({expanded:true},i.data("wijexpander-options")))});e.find(".accordion").wijaccordion({header:"h3",selectedIndexChanged:function(j,i){a(j.target).find(".ui-accordion-content").eq(i.newIndex).nos().initOnShow()}});break;case"validate":var d=this,g=f[0]||{};if(!d.is("form")){d=d.find("form")}require(["jquery-validate"],function(){d.validate(a.extend({},g,{errorClass:"ui-state-error",success:true,ignore:"input[type=hidden]"}))});break;case"ajax":var d=this;if(!d.is("form")){d=d.find("form")}require(["jquery-form"],function(){d.ajaxForm({dataType:"json",success:function(i){a.nos.ajax.success(i);d.triggerHandler("ajax_success",[i])},error:function(){a.nos.notify("An error occured","error")}})});break}return this},initOnShow:function(){var d=Array.prototype.slice.call(arguments),f="show";if(d.length>0&&a.inArray(d[0],["init","show"])!==-1){f=d.shift()}switch(f){case"init":var e=d[0];if(!a.isFunction(e)){return}this.each(function(){var g=a(this);if(g.is(":visible")){e.call(g)}else{g.addClass("nos-init-on-show").data("nos-init-on-show",e)}});break;case"show":this.find(".nos-init-on-show:visible").each(function(){var g=a(this),h=g.data("nos-init-on-show");g.removeClass("nos-init-on-show");h.call(this)});break}return this},dialog:function(){var f=Array.prototype.slice.call(arguments),k="open";if(f.length>0&&a.inArray(f[0],["open","close"])!==-1){k=f.shift()}switch(k){case"open":var e=f[0]||{};if(e.destroyOnClose){var g=e.close;e.close=function(){if(a.isFunction(g)){g.apply(this,arguments)}a(this).wijdialog("destroy").remove()}}e=a.extend(true,{},{width:window.innerWidth-200,height:window.innerHeight-100,modal:true,captionButtons:{pin:{visible:false},refresh:{visible:e.contentUrl!=null&&!e.ajax},toggle:{visible:false},minimize:{visible:false},maximize:{visible:false}}},e);var d=a.nos.$noviusos.ostabs?a.nos.$noviusos.ostabs("current").panel:a("body");var i=a(document.createElement("div")).appendTo(d);if(e.content!==c){i.append(e.content)}var h=true;if(e.ajax){var j=e.contentUrl;delete e.contentUrl;e.autoOpen=false;i.wijdialog(e);a.ajax({url:j,type:"GET",dataType:"html",complete:function(n,l,o){o=n.responseText;if(n.isResolved()){n.done(function(q){o=q});try{var m=a.parseJSON(o);h=false}catch(p){}if(h){i.wijdialog("open")}else{i.empty();i.wijdialog("destroy");i.remove();a.nos.ajax.success(m)}i.html(o)}}})}else{i.wijdialog(e)}if(h){if(a.isFunction(e.onLoad)){e.onLoad()}i.bind("wijdialogclose",function(l,m){i.closest(".ui-dialog").hide().appendTo(d)})}return i;break;case"close":this.closest(":wijmo-wijdialog").wijdialog("close").wijdialog("destroy").remove();break}return this},tab:function(){var d=Array.prototype.slice.call(arguments),i="open";if(d.length>0&&a.inArray(d[0],["open","close"])!==-1){i=d.shift()}switch(i){case"open":var g=d[0]||{},f=d[1]||{},e=this.closest(".ui-dialog-content").size();if(e){this.dialog(a.extend({contentUrl:g.url,ajax:!g.iframe,title:g.label},f))}else{a.nos.tabs.add(g)}break;case"close":var h=this.closest(":wijmo-wijdialog");if(h.size()){this.dialog("close")}else{a.nos.tabs.close()}break}return this}});a.fn.nos=function(){return b(this)};return b});
...
define("jquery-nos-ostabs",["jquery","log","order!jquery-ui","order!wijmo-open","order!wijmo-complete","order!jquery-nos","order!jquery-nos-loadspinner"],function(a){var b=void (0);a.widget("nos.ostabs",{options:{initTabs:[],newTab:"appsTab",trayTabs:[],appsTab:{},fx:null,labelMinWidth:100,labelMaxWidth:200,selected:null,texts:{scrollLeft:"Scroll left",scrollRight:"Scroll right",newTab:"New tab",removeTab:"Remove tab",closeTab:"Close tab",reloadTab:"Reload tab",spinner:"Loading..."},add:null,remove:null,select:null,show:null,drag:null},tabId:0,stackOpening:[],_create:function(){var c=this,d=c.options;c._tabify(true);a(window).resize(function(){if(d.selected){c._firePanelEvent(c.panels.eq(d.selected),{event:"resizePanel"})}})},_setOption:function(d,e){var c=this;if(d=="selected"){c.select(e)}else{c.options[d]=e;c._tabify()}},_getNextTabId:function(){var c=this;return ++c.tabId},_tabId:function(c){var d=this;return a(c).data("panelid.tabs")||"nos-ostabs-"+d._getNextTabId()},_sanitizeSelector:function(c){return c.replace(/:/g,"\\:")},_ui:function(c,d){var e=this,f=e.lis.index(c);if(d===b){d=e.element.find(e._sanitizeSelector(e.anchors[f].hash))[0]}return{tab:c,panel:d,index:f}},_cleanup:function(){var c=this;c.lis.filter(".ui-state-processing").removeClass("ui-state-processing").find("span.nos-ostabs-icon").removeClass("nos-ostabs-loader").each(function(){if(a.isFunction(a.fn.loadspinner)){a(this).loadspinner("destroy")}}).html(" ").parent().find("span.nos-ostabs-label").each(function(){var e=a(this),d=e.data("label.tabs");if(d){e.html(d).removeData("label.tabs")}});return c},_tabify:function(n){var p=this,f=p.options,d=/^#.+/;if(n){p.element.addClass("nos-ostabs ui-widget ui-widget-content");p.uiOstabsHeader=a("<div></div>").addClass("ui-widget-header").appendTo(p.element);p.uiOstabsSuperPanel=a("<div></div>").addClass("nos-ostabs-tabs").appendTo(p.uiOstabsHeader);p.uiOstabsTray=a("<ul></ul>").addClass("nos-ostabs-tray nos-ostabs-nav");if(a.isArray(f.trayTabs)){a.each(f.trayTabs,function(o,q){if(a.isPlainObject(q)){p._add(q,p.uiOstabsTray).addClass("nos-ostabs-tray")}})}p.uiOstabsTray.prependTo(p.uiOstabsHeader);if(a.isPlainObject(f.appsTab)){p.uiOstabsAppsTab=a("<ul></ul>").addClass("nos-ostabs-appstab  nos-ostabs-nav").prependTo(p.uiOstabsHeader);p._add(f.appsTab,p.uiOstabsAppsTab).addClass("nos-ostabs-appstab").removeClass("ui-state-default")}else{p.uiOstabsAppsTab=a("<ul></ul>")}p.uiOstabsSuperPanel.css("left",p.uiOstabsAppsTab.outerWidth(true)+25).css("right",p.uiOstabsTray.outerWidth(true)+35).wijsuperpanel({allowResize:false,autoRefresh:true,hScroller:{scrollBarVisibility:"hidden",scrollMode:"buttons",increaseButtonPosition:{my:"left bottom",at:"right bottom"},decreaseButtonPosition:{my:"right bottom",at:"left bottom"}},vScroller:{scrollBarVisibility:"hidden"},showRounder:false}).removeClass("ui-widget ui-widget-content").css("overflow","visible");p.uiOstabsSuperPanelContent=p.uiOstabsSuperPanel.wijsuperpanel("getContentElement");p.uiOstabsTabs=a("<ul></ul>").appendTo(p.uiOstabsSuperPanelContent);if(a.isArray(f.initTabs)){a.each(f.initTabs,function(o,q){p._add(q)})}var e=f.newTab;if(a.isPlainObject(e)){e=a.extend({label:f.texts.newTab,iconClasses:"ui-icon ui-icon-circle-plus"},e)}else{if(e&&a.isPlainObject(f.appsTab)){e=a.extend({},f.appsTab,{label:f.texts.newTab,iconClasses:"ui-icon ui-icon-circle-plus",iconUrl:"",iconSize:16})}else{e=false}}if(e){p.uiOstabsNewTab=p._add(e).addClass("nos-ostabs-newtab")}else{p.uiOstabsNewTab=a("<li>/<li>")}p.uiOstabsNewTab.hover(function(){a(this).find(".nos-ostabs-label").css("display","inline-block");p._tabsWidth()},function(){a(this).find(".nos-ostabs-label").css("display","none");p._tabsWidth()});p.uiOstabsTabs.sortable({items:"li:not(.nos-ostabs-newtab)",appendTo:p.uiOstabsSuperPanelContent,cursor:"move",delay:250,scroll:false,helper:"clone",tolerance:"pointer",axis:"x",zIndex:100000,placeholder:"ui-state-highlight",forcePlaceholderSize:true,start:function(){p.sorting=true},stop:function(){p.sorting=false},update:function(){p._trigger("drag",null);p.lis=p.uiOstabsAppsTab.add(p.uiOstabsTray).add(p.uiOstabsTabs).find("li:has(a[href])");p.anchors=p.lis.map(function(q){var o=a("a",this)[0];p.element.find(p._sanitizeSelector(o.hash)).find(".nos-ostabs-panel-content").data("nos-ostabs-index",q);return o});p._tabsWidth()}});p.tabsWidth=p.uiOstabsSuperPanelContent.width();p.labelWidth=f.labelMaxWidth;p.uiOstabsTabs.width(p.tabsWidth)}p.lis=p.uiOstabsAppsTab.add(p.uiOstabsTray).add(p.uiOstabsTabs).find("li:has(a[href])");p.anchors=p.lis.map(function(){return a("a",this)[0]});p.panels=a([]);p.anchors.each(function(r,o){var q=a(o).attr("href");var s=q.split("#")[0],t;if(s&&(s===location.toString().split("#")[0]||(t=a("base")[0])&&s===t.href)){q=o.hash;o.href=q}if(d.test(q)){p.panels=p.panels.add(p.element.find(p._sanitizeSelector(q)))}else{if(q&&q!=="#"){a.data(o,"href.tabs",q);a.data(o,"load.tabs",q.replace(/#.*$/,""));var v=p._tabId(o);o.href="#"+v;var u=p.element.find("#"+v);if(!u.length){p.lis.eq(r);u=a("<div></div>").attr("id",v).addClass("nos-ostabs-panel ui-widget-content ui-corner-bottom nos-ostabs-hide").appendTo(p.element);p._actions(u,r)}p.panels=p.panels.add(u)}}});if(n){p.uiOstabsTray.add(p.uiOstabsAppsTab).addClass("nos-ostabs-nav ui-helper-reset ui-helper-clearfix");p.lis.addClass("ui-corner-top");p.panels.addClass("nos-ostabs-panel ui-widget-content");if(f.selected===b){if(location.hash){p.anchors.each(function(q,o){if(o.hash==location.hash){f.selected=q;return false}})}if(typeof f.selected!=="number"&&p.lis.filter(".nos-ostabs-selected").length){f.selected=p.lis.index(p.lis.filter(".nos-ostabs-selected"))}f.selected=f.selected||(p.lis.length?0:-1)}f.selected=((f.selected>=0&&p.anchors[f.selected])||f.selected<0)?f.selected:0;p.panels.addClass("nos-ostabs-hide");p.lis.removeClass("nos-ostabs-selected ui-state-active");if(f.selected>=0&&p.anchors.length){p.element.find(p._sanitizeSelector(p.anchors[f.selected].hash)).removeClass("nos-ostabs-hide");p.lis.eq(f.selected).addClass("nos-ostabs-selected ui-state-active ui-state-open");p.element.queue("tabs",function(){p._trigger("show",null,p._ui(p.lis[f.selected]))});p.title(f.selected,p.title(f.selected));p._load(f.selected)}a(window).bind("unload",function(){p.lis.add(p.anchors).unbind(".tabs");p.lis=p.anchors=p.panels=null})}else{f.selected=p.lis.index(p.lis.filter(".nos-ostabs-selected"))}p.anchors.removeData("cache.tabs");p.lis.add(p.anchors).unbind(".tabs");var h=function(q,o){o.addClass("ui-state-"+q)};var k=function(q,o){o.removeClass("ui-state-"+q)};p.lis.bind("mouseover.tabs",function(){h("hover",a(this))});p.lis.bind("mouseout.tabs",function(){k("hover",a(this))});p.anchors.bind("focus.tabs",function(){h("focus",a(this).closest("li"))});p.anchors.bind("blur.tabs",function(){k("focus",a(this).closest("li"))});var c,i;if(f.fx){if(a.isArray(f.fx)){c=f.fx[0];i=f.fx[1]}else{c=i=f.fx}}function g(o,q){o.css("display","");if(!a.support.opacity&&q.opacity){o[0].style.removeAttribute("filter")}}function j(r){var q=r.data("callbacks.ostabs"),o=r.find(".nos-dispatcher");if(a.isPlainObject(q)){a.each(q,function(s,t){p._firePanelEvent(o,t)});q={}}}var l=i?function(o,q){var r=a(o).closest("li").addClass("nos-ostabs-selected ui-state-active");q.hide().removeClass("nos-ostabs-hide").animate(i,i.duration||"normal",function(){if(r.hasClass("nos-ostabs-newtab")){p._tabsWidth()}p.uiOstabsSuperPanel.wijsuperpanel("scrollChildIntoView",r.find("a"));g(q,i);j(q);p._firePanelEvent(q,{event:"showPanel"})})}:function(o,q){var r=a(o).closest("li");if(r.hasClass("nos-ostabs-newtab")){p._tabsWidth()}p.uiOstabsSuperPanel.wijsuperpanel("scrollChildIntoView",r.find("a"));r.addClass("nos-ostabs-selected ui-state-active");q.removeClass("nos-ostabs-hide");j(q);p._firePanelEvent(q,{event:"showPanel"})};var m=c?function(q,o){o.animate(c,c.duration||"normal",function(){if(p.uiOstabsNewTab.hasClass("ui-state-active")){p._tabsWidth()}p.lis.removeClass("nos-ostabs-selected ui-state-active");o.addClass("nos-ostabs-hide");g(o,c);p.element.dequeue("tabs");o.trigger("hidePanel.ostabs")})}:function(q,o){if(p.uiOstabsNewTab.hasClass("ui-state-active")){p._tabsWidth()}p.lis.removeClass("nos-ostabs-selected ui-state-active");o.addClass("nos-ostabs-hide");p.element.dequeue("tabs");o.trigger("hidePanel.ostabs")};p.anchors.bind("click.tabs",function(){var q=this,s=a(q).closest("li"),o=p.panels.filter(":not(.nos-ostabs-hide)"),r=p.element.find(p._sanitizeSelector(q.hash));s.addClass("ui-state-open");p.uiOstabsNewTab.removeClass("ui-state-open");f.selected=p.anchors.index(this);if(s.hasClass("nos-ostabs-selected")||s.hasClass("ui-state-processing")||p.panels.filter(":animated").length||p._trigger("select",null,p._ui(s[0],r[0]))===false){this.blur();return false}p._abort();if(r.length){if(o.length){p.element.queue("tabs",function(){m(q,o)})}p.element.queue("tabs",function(){p.stackOpening.push(q);l(q,r)});a("title").text(s.find(".nos-ostabs-label").text());p._load(p.anchors.index(this))}else{throw"jQuery UI Tabs: Mismatching fragment identifier."}this.blur()});p.anchors.bind("click.tabs",function(){return false});p._tabsWidth()},_getIndex:function(d){var c=this;if(typeof d=="string"){d=c.anchors.index(c.anchors.filter("[href$="+d+"]"))}return d},_actions:function(p,h){var n=this,e=n.options;var j=n.lis.eq(h),i=n.anchors.eq(h);var f=a("<div></div>").addClass("nos-ostabs-actions ui-state-active").prependTo(p);var l=a("<div></div>").addClass("nos-ostabs-actions-links").prependTo(f);var d=j.not(".nos-ostabs-tray").not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length;var k=j.not(".nos-ostabs-appstab").length;var g=i.data("iframe.tabs");if(k){var m=a('<a href="#"></a>').addClass("nos-ostabs-close").click(function(){n.remove(n.lis.index(j));return false}).appendTo(l);a("<span></span>").addClass("ui-icon ui-icon-closethick").text(d?e.texts.removeTab:e.texts.closeTab).appendTo(m);a("<span></span>").text(d?e.texts.removeTab:e.texts.closeTab).appendTo(m)}if(g){var c=a('<a href="#"></a>').addClass("nos-ostabs-reload").click(function(){var o=p.find("iframe.nos-ostabs-panel-content");if(o!==b){o.attr("src",o.attr("src"))}return false}).text(e.texts.reloadTab).appendTo(l);a("<span></span>").addClass("ui-icon ui-icon-refresh").text(e.texts.reloadTab).appendTo(c)}},_tabsWidth:function(){var c=this,g=c.options,e=0;c.uiOstabsTabs.width(10000000);c.uiOstabsTabs.find("li").each(function(){e+=a(this).outerWidth(true)});c.uiOstabsTabs.width(e);var d=c.uiOstabsTabs.find(".nos-ostabs-label:visible").length,f;if(c.tabsWidth<c.uiOstabsTabs.width()){while(c.tabsWidth<c.uiOstabsTabs.width()&&c.labelWidth>g.labelMinWidth){f=c.labelWidth-g.labelMinWidth;f=f>10?10:f;e=e-d*f;c.uiOstabsTabs.width(e);c.labelWidth=c.labelWidth-f;a("head .tabswidth").remove();a('<style type="text/css" class="tabswidth">.nos-ostabs .ui-widget-header .nos-ostabs-label {width : '+c.labelWidth+"px !important;}</style>").appendTo("head")}}else{do{f=g.labelMaxWidth-c.labelWidth;f=f>10?10:f;if(c.tabsWidth>(e+d*10)){e=e+d*f;c.uiOstabsTabs.width(e);c.labelWidth=c.labelWidth+f;a("head .tabswidth").remove();a('<style type="text/css" class="tabswidth">.nos-ostabs .ui-widget-header .nos-ostabs-label {width : '+c.labelWidth+"px !important;}</style>").appendTo("head")}}while(c.tabsWidth>(e+d*10)&&c.labelWidth<g.labelMaxWidth)}if(c.tabsWidth<c.uiOstabsTabs.width()){c.uiOstabsSuperPanel.find(".wijmo-wijsuperpanel-buttonleft, .wijmo-wijsuperpanel-buttonright").show()}else{c.uiOstabsSuperPanel.find(".wijmo-wijsuperpanel-buttonleft, .wijmo-wijsuperpanel-buttonright").hide()}},add:function(e,d){var c=this;if(!a.isPlainObject(e)||e.url===b){return false}if(d===b){d=c.anchors.length-1}var f=c._add(e);if(d<c.lis.length){f.insertBefore(c.lis[d])}else{d=c.lis.eq(f)}c.uiOstabsTabs.sortable("refresh");c._tabify();if(c.anchors.length==1){c.select(0)}c._trigger("add",e,c._ui(f[0]));return d},_add:function(h,i){var e=this;if(!a.isPlainObject(h)||h.url===b){return false}i=i||e.uiOstabsTabs;h=a.extend({url:"",iframe:false,label:"",labelDisplay:true,iconClasses:"ui-icon ui-icon-document",iconUrl:"",iconSize:16,panelId:false},h);var d=a('<a href="'+h.url+'"></a>');if(h.iframe){d.data("iframe.tabs",true)}if(h.panelId){d.data("panelid.tabs",h.panelId)}var g=e._icon(h).appendTo(d);var f=a("<span></span>").addClass("nos-ostabs-label").text(h.label?h.label:"New tab").appendTo(d);if(!h.labelDisplay){f.hide()}var c=a("<li></li>").append(d).attr("title",h.label).addClass("ui-corner-top ui-state-default").data("ui-ostab",h).appendTo(i);if(!isNaN(h.iconSize)&&h.iconSize!==16&&i!==e.uiOstabsTabs){c.css({height:(h.iconSize+4)+"px",bottom:(h.iconSize-35)+"px"});g.css("top","2px")}return c},remove:function(d){var c=this;d=c._getIndex(d);var j=c.lis.eq(d),g=c.anchors.eq(d),h=c.element.find(c._sanitizeSelector(c.anchors[d].hash));if(d==0&&!j.hasClass("nos-ostabs-selected")){var f=c.lis.filter(".nos-ostabs-newtab");if(f.hasClass("nos-ostabs-selected")){j=f}}if(j.not(".nos-ostabs-tray").not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length){j.remove();h.remove()}j.removeClass("ui-state-active ui-state-open");for(var e=0;e<c.stackOpening.length;e++){if(c.stackOpening[e]===g.get(0)){c.stackOpening.splice(e,1)}}if(c.stackOpening.length){c.select(c.anchors.index(c.stackOpening[c.stackOpening.length-1]))}else{c.select(0)}if(j.not(".nos-ostabs-appstab").not(".nos-ostabs-newtab").length){a("> *",h).not(".nos-ostabs-actions").remove()}h.addClass("nos-ostabs-hide").removeData("callbacks.ostabs");j.removeClass("nos-ostabs-selected");c._tabify();c._trigger("remove",null,c._ui(j[0],h[0]));return c},title:function(d,f){var c=this,e=c.options;d=c._getIndex(d);var g=c.lis.eq(d);if(f===b){return g.find(".nos-ostabs-label").text()}else{g.find(".nos-ostabs-label").text(f);if(e.selected==d){a("title").text(f)}c._trigger("title",null,c._ui(g[0]));return c}},update:function(e,f){var d=this;e=d._getIndex(e);if(!a.isPlainObject(f)){return false}var h=d.lis.eq(e);f=a.extend({},h.data("ui-ostab"),f);if(d.options.selected==e){a("title").text(f.label)}var c=d._add(f),g=c.find("a");h.data("ui-ostab",f).attr("title",f.label||"").addClass(c.attr("class")).css({height:c.css("height"),bottom:c.css("bottom")}).find("a").empty().append(g.children());c.remove();return d},_icon:function(d){var c=a("<span></span>").addClass("nos-ostabs-icon");if(d.iconUrl){c.css("background-image",'url("'+d.iconUrl+'")')}else{c.addClass(d.iconClasses)}if(!isNaN(d.iconSize)&&d.iconSize!==16){c.css({width:d.iconSize+"px",height:d.iconSize+"px",lineHeight:d.iconSize+"px",top:(d.iconSize>22?22-d.iconSize:10-d.iconSize/2)+"px"})}return c},select:function(d){var c=this;d=c._getIndex(d);if(d==-1){return c}c.anchors.eq(d).trigger("click.tabs");return c},_load:function(g){var e=this;g=e._getIndex(g);var j=e.options,d=e.anchors.eq(g)[0],f=a.data(d,"load.tabs"),h=a.data(d,"iframe.tabs");e._abort();if((!f&&h)||e.element.queue("tabs").length!==0&&a.data(d,"cache.tabs")){e.element.dequeue("tabs");return}var c=e.element.find(e._sanitizeSelector(d.hash)),i=a("> *",c).not(".nos-ostabs-actions").length;if(i!==0){e.element.dequeue("tabs");return}e.lis.eq(g).addClass("ui-state-processing");a("span.nos-ostabs-label",d).each(function(){var k=a(this);k.data("label.tabs",k.html()).html(k.data("label.tabs")?j.texts.spinner:"")});if(a.isFunction(a.fn.loadspinner)){a("span.nos-ostabs-icon",d).each(function(){var k=a(this);k.addClass("ui-state-processing").loadspinner({diameter:k.width(),scaling:true})})}if(!h){e.xhr=a.ajax({url:f,success:function(k){a("<div></div>").addClass("nos-ostabs-panel-content nos-dispatcher").data("nos-ostabs-index",g).prependTo(c.data("callbacks.ostabs",{})).html(k);a.data(d,"cache.tabs",true)},complete:function(){e._cleanup();e._trigger("load",null,e._ui(e.lis[g]))}})}else{a("<iframe "+(a.browser.msie?'allowTransparency="true" ':"")+'src="'+f+'" frameborder="0"></iframe>').data("nos-ostabs-index",g).addClass("nos-ostabs-panel-content nos-dispatcher").bind("load",function(){e._cleanup();e._trigger("load",null,e._ui(e.lis[g]))}).prependTo(c.data("callbacks.ostabs",{}));a.data(d,"cache.tabs",true)}e.element.dequeue("tabs");return e},_abort:function(){var c=this;c.element.queue([]);c.panels.stop(false,true);c.element.queue("tabs",c.element.queue("tabs").splice(-2,2));c._cleanup();return c},tabs:function(){var c=this,d=[];c.uiOstabsTabs.find("li:not(.nos-ostabs-newtab)").each(function(){d.push(a(this).data("ui-ostab"))});return d},dispatchEvent:function(d){var c=this,e=c.options;if(!a.isPlainObject(d)){d={event:d}}a.each(c.panels,function(f){var h=a(this);if(f===e.selected){c._firePanelEvent(h,d)}else{var g=h.data("callbacks.ostabs");if(a.isPlainObject(g)){g[d.event+(d.type?"."+d.type:"")]=d}}});return c},_firePanelEvent:function(c,d){c=c.is(".nos-dispatcher")?c:c.find(".nos-dispatcher");var f=a.Event(d.event,{namespace:d.type||null,data:d.data||null});if(c.is("iframe")){if(c[0].contentDocument.$){c[0].contentDocument.$("body").trigger(f)}}else{try{c.trigger(f)}catch(f){log(f)}}},current:function(){var c=this,d=c.options;return{tab:a(c.lis.get(d.selected)),panel:a(c.panels.get(d.selected)),index:d.selected}}});a(function(){var c=a("#noviusos");a.extend(a.nos,{$noviusos:c,initialize:function(f){var d=this;var e=function(g){d.tabs.save()};a.extend(f,{add:e,remove:e,select:e,show:e,drag:e});if(f.user_configuration["tabs"]){if(!f.options){f.options={}}f.initTabs=f.user_configuration["tabs"]["tabs"];f.selected=f.user_configuration["tabs"]["selected"]}c.ostabs(f)},tabs:{current:function(d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos(window.frameElement).data("nos-ostabs-index")}if(a.isNumeric(d)){return d}if(a.type(d)==="object"&&a.isFunction(d.parents)){return d.parents(".nos-ostabs-panel-content").data("nos-ostabs-index")}if(c.length){return c.ostabs("current").index}return false},add:function(f,d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.add(f,d)}var e;if(d!==b&&d!==true){e=this.current(d)+1}if(c.length){e=c.ostabs("add",f,e);return c.ostabs("select",e)}else{if(f.url){window.open(f.url)}}return false},open:function(f,d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.add(f,d)}if(c.length){var e=c.ostabs("tabs"),g=false;a.each(e,function(j,h){if(h.url===f.url){g=j;return false}});if(g!==false){return c.ostabs("select",g+4)}else{return this.add(f,d)}}else{if(f.url){window.open(f.url)}}return false},update:function(d,e){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.update(this.current(),d,e)}if(e==null){e=d;d=null}if(a.type(d)==="object"&&a.isFunction(d.parents)&&d.closest(".ui-dialog-content").size()){return true}d=this.current(d);if(c.length){c.ostabs("update",d,e)}return true},close:function(d){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.close(this.current())}d=this.current(d);if(c.length){c.ostabs("remove",d)}return true},replace:function(e){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.replace(e)}var d=this.current();this.add({url:e},false);this.close(d);return true},save:function(){if(window.parent!=window&&window.parent.$nos){return window.parent.$nos.nos.tabs.save()}if(c.length){$nos.nos.saveUserConfiguration("tabs",{selected:c.ostabs("option","selected"),tabs:c.ostabs("tabs")})}}}})});return a});
