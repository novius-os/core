...
define("jquery-nos-listgrid",["jquery","jquery-nos","jquery-ui.widget","wijmo.wijgrid"],function(a){a.widget("nos.noslistgrid",a.wijmo.wijgrid,{options:{noCellsSelected:false,culture:a.nosLang.substr(0,2),loadingText:"Loading..."},_init:function(){var e=this,f=e.options,b,d,c;if(a.isFunction(f.columnResized)){b=f.columnResized}f.columnResized=function(i,g){var h=e.option("columns");a.extend(h[g.column.thX],{ensurePxWidth:true,width:g.column.width});e.setSize(e.outerDiv.parent().width());if(a.isFunction(b)){b.apply(this,arguments)}};if(a.isFunction(f.dataLoaded)){d=f.ajaxError}f.ajaxError=function(i,g){var h=g.XMLHttpRequest;if(h.status==403){a(this).nosAjaxError(h)}if(a.isFunction(d)){d.apply(this,arguments)}};if(f.noCellsSelected){if(a.isFunction(f.cellStyleFormatter)){c=f.cellStyleFormatter}f.cellStyleFormatter=function(g){if(a.isFunction(c)){c.apply(this,arguments)}if(g.$cell.is("td")){g.$cell.removeClass("ui-state-highlight ui-state-default")}}}e._super()}});return a});
