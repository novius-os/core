            ...
            function($media, $params) use (&$replaces) {
                if (empty($media)) {
                    ...
                    if (!empty($params['width']) && !empty($params['height']) && ($params['width'] != $media->media_width || $params['height'] != $media->media_height)) {
                        $replaces[$params['url'].'"'] = \Uri::base(true).$media->urlResized($params['width'], $params['height']).'" width="'.$params['width'].'" height="'.$params['height'].'" data-media-id="'.$media->id.'"';
                    } else {
                        $replaces[$params['url'].'"'] = \Uri::base(true).$media->url().'" data-media-id="'.$media->id.'"';
                    }
                }
            }
        );
