...
require(
    ...
    function($) {
        $(function() {
            ...
            $content.find('select[context_invariant_field]').each(function() {
                var $select = $(this);
                $select.closest('.wijmo-wijdropdown').find('a').click(function() {
                    if ($select.wijdropdown('option', 'disabled')) {
                        var $div = $('<div><p></p><ul></ul></div>')
                                .find('p')
                                .text('When you modify the value of this field, the change is also applied to the following contexts/languages/sites:')
                                .end(),
                            $ul = $div.find('ul'),
                            contexts = $select.data('other-contexts');
                        ...
                        $content.nosDialog({
                            title: 'This field is common to all contexts/languages/sites',
                            content: $div,
                            width: 500,
                            ...
                            buttons: [
                                {
                                    ...
                                    click: function () {
                                        $select.wijdropdown('option', 'disabled', false);
                                        $(this).wijdialog("close");
                                    }
                                },
                                {
                                    text: 'Cancel, I won\'t modify it',
                                    click: function () {
                                        $(this).wijdialog("close");
                                    }
                                }
