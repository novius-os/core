...
<div id="<?= $id ?>">
    ...
    $fieldset->form()->set_config(
        'field_template',
        "\t\t<tr><th class=\"{error_class}\">{label}{required}</th><td class=\"{error_class}\">{field} {error_msg}</td><td class=\"use_default\">{default}</td></tr>\n"
    );
                ...
                function ($field) use ($item, $nugget_db, $nugget) {
                    ...
                    if (in_array($field_name, array(\Nos\DataCatcher::TYPE_TITLE, \Nos\DataCatcher::TYPE_TEXT)) && array_key_exists($field_name, $nugget) && !empty($sharable_property)) {
                        ...
                        $template = str_replace(
                            '{default}',
                            '<input type="checkbox" name="default['.$field_name.']" id="'.$id.'" class="nos-datacatchers-nugget-checkbox" '.$checked.' /> <label for="'.$id.'">'.$label.'</label>',
                            $template
                        );
                    } else {
